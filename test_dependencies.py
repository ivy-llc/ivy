...
def test_imports(fname, assert_version, update_versions):
    ...
    for line_num, (mod_name, expected_version) in enumerate(mod_names_n_versions):
        ...
        if detected_version and expected_version:
            ...
            if detected_version:
                msg = ("{} detected version: {}, but no expected version "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help=("Comma separated filepaths of all text files to check. "
              "Spaces are ignored."),
    )
    parser.add_argument(
        "-amv",
        ...
        action="store_true",
        help=("Whether to assert that all module versions match those lists in the "
