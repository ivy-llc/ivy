...
# test lists
framework_tests_to_run = {'jax':list(), 'mxnet':list(), 'numpy':list(), 'torch':list(), 'tensorflow':list()}
framework_tests_to_skip = {'jax':list(), 'mxnet':list(), 'numpy':list(), 'torch':list(), 'tensorflow':list()}
...
#method_name #failing for torch for the following reason
        ...
        for framework in framework_tests_to_run:
            ...
            for s in contents:
                if s == '':
                    continue            
                if ('#' not in s) or ('#' in s and not (framework in s.lower()) and any(f in s.lower() for f in framework_tests_to_run)):
...
# prune tests to skip
    framework_tests_to_skip[framework] = [tts for tts in framework_tests_to_skip[framework] if not max([tts in ttr for ttr in framework_tests_to_run[framework]])]



# save to file
for framework in framework_tests_to_run:
    with open(os.path.join(this_dir, '.array_api_tests_k_flag_'+framework), 'w+') as file:
        file.write('(' + ' or '.join(framework_tests_to_run[framework]) + ') and not (' + ' or '.join(framework_tests_to_skip[framework]) + ')')
