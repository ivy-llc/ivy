...
if FILTER_UNDEFINED_DTYPES:
    ...
    floating_dtypes = floating_dtypes.filter(
        lambda x: not isinstance(x, _UndefinedStub)
    )
    numeric_dtypes = numeric_dtypes.filter(lambda x: not isinstance(x, _UndefinedStub))
    integer_or_boolean_dtypes = integer_or_boolean_dtypes.filter(
        lambda x: not isinstance(x, _UndefinedStub)
    )
...
if FILTER_UNDEFINED_DTYPES:
    _promotable_dtypes = [
        (d1, d2)
        for d1, d2 in _promotable_dtypes
        if not isinstance(d1, _UndefinedStub) or not isinstance(d2, _UndefinedStub)
    ]
promotable_dtypes: List[Tuple[DataType, DataType]] = sorted(
    _promotable_dtypes, key=_dtypes_sorter
)
    ...
    *,
    ...
    for samples in category_samples.values():
        if len(samples) > 0:
            ...
            strats.append(strat)
    if (
        len(category_samples[dh.uint_dtypes]) > 0
        and len(category_samples[dh.int_dtypes]) > 0
    ):
        mixed_samples = (
            category_samples[dh.uint_dtypes] + category_samples[dh.int_dtypes]
        )
...
elementwise_functions_names = shared(
    sampled_from([f.__name__ for f in category_to_funcs["elementwise"]])
)
...
elementwise_function_objects = elementwise_functions_names.map(lambda i: getattr(xp, i))
array_functions = elementwise_function_objects
multiarg_array_functions = multiarg_array_functions_names.map(lambda i: getattr(xp, i))
...
one_d_shapes = xps.array_shapes(
    min_dims=1, max_dims=1, min_side=0, max_side=SQRT_MAX_ARRAY_SIZE
)
...
def invertible_matrices(draw, dtypes=xps.floating_dtypes(), stack_shapes=shapes()):
    ...
    n = draw(
        integers(0, SQRT_MAX_ARRAY_SIZE),
    )
    stack_shape = draw(stack_shapes)
...
def integer_indices(draw, sizes):
    ...
    m, M = dh.dtype_ranges[dtype]
    if m <= idx <= M:
        return draw(one_of(just(idx), just(full((), idx, dtype=dtype))))
    return idx
...
def slices(draw, sizes):
    ...
    step = draw(
        one_of(integers(-max_step_size, -1), integers(1, max_step_size), none())
    )
...
def multiaxis_indices(draw, shapes):
    ...
    for i in range(n_entries):
        size = shape[k]
        idx = draw(one_of(integer_indices(just(size)), slices(just(size)), just(...)))
        if idx is ... and k >= 0:
    ...
    if not res_has_ellipsis:
        if n_entries < len(shape):
            ...
            extra = draw(
                lists(
                    one_of(integer_indices(sizes), slices(sizes)),
                    min_size=0,
                    max_size=3,
                )
            )
