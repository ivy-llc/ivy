...
def _test_stacks(
    ...
    *args,
    ...
    true_val=None,
    matrix_axes=(-2, -1),
    assert_equal=assert_exactly_equal,
    **kw,
):
    """
...
def _test_namedtuple(res, fields, func_name):
    ...
    assert len(res) == len(
        fields
    ), f"{func_name}() result tuple not the correct length (should have {len(fields)} elements)"
    for i, field in enumerate(fields):
        assert hasattr(
            res, field
        ), f"{func_name}() result namedtuple doesn't have the '{field}' field"
        assert res[i] is getattr(
            res, field
        ), f"{func_name}() result namedtuple '{field}' field is not in position {i}"


@pytest.mark.xp_extension("linalg")
@given(x=positive_definite_matrices(), kw=kwargs(upper=booleans()))
...
def test_cross(x1_x2_kw):
    ...
    assert res.dtype == dh.result_type(
        x1.dtype, x2.dtype
    ), "cross() did not return the correct dtype"
    assert res.shape == shape, "cross() did not return the correct shape"

    def exact_cross(a, b):
        assert (
            a.shape == b.shape == (3,)
        ), "Invalid cross() stack shapes. This indicates a bug in the test suite."
        return asarray(
            [
                a[1] * b[2] - a[2] * b[1],
                a[2] * b[0] - a[0] * b[2],
    ...
    # vectors.
    _test_stacks(
        linalg.cross, x1, x2, dims=1, matrix_axes=(axis,), res=res, true_val=exact_cross
    )
...
def test_diagonal(x, kw):
    ...
    assert res.shape == (
        *x.shape[:-2],
        diag_size,
    ), "diagonal() returned the wrong shape"
...
def test_eigh(x):
    ...
    assert (
        eigenvalues.dtype == x.dtype
    ), "eigh().eigenvalues did not return the correct dtype"
    assert (
        eigenvalues.shape == x.shape[:-1]
    ), "eigh().eigenvalues did not return the correct shape"

    assert (
        eigenvectors.dtype == x.dtype
    ), "eigh().eigenvectors did not return the correct dtype"
    assert (
        eigenvectors.shape == x.shape
    ), "eigh().eigenvectors did not return the correct shape"

    _test_stacks(lambda x: linalg.eigh(x).eigenvalues, x, res=eigenvalues, dims=1)
    _test_stacks(lambda x: linalg.eigh(x).eigenvectors, x, res=eigenvectors, dims=2)
...
@given(*two_mutual_arrays(dh.numeric_dtypes))
def test_matmul(x1, x2):
    ...
    if (
        ...
        ph.raises(
            Exception,
            lambda: _array_module.matmul(x1, x2),
...
def test_matrix_norm(x, kw):
    ...
    ), f"matrix_norm({keepdims=}) did not return the correct shape"
    assert res.dtype == x.dtype, "matrix_norm() did not return the correct dtype"

    _test_stacks(linalg.matrix_norm, x, **kw, dims=2 if keepdims else 0, res=res)
...
@given(
    ...
    x=matrix_power_n.flatmap(
        lambda n: invertible_matrices()
        if n < 0
        else xps.arrays(dtype=xps.floating_dtypes(), shape=square_matrix_shapes)
    ),
    n=matrix_power_n,
)
...
@pytest.mark.xp_extension("linalg")
@given(x=finite_matrices(shape=rtol_shared_matrix_shapes), kw=kwargs(rtol=rtols))
...
def test_matrix_transpose(x):
    ...
    true_val = lambda a: _array_module.asarray(
        [[a[i, j] for i in range(a.shape[0])] for j in range(a.shape[1])], dtype=a.dtype
    )
    shape = list(x.shape)
...
@given(
    *two_mutual_arrays(
        dtypes=dh.numeric_dtypes, two_shapes=tuples(one_d_shapes, one_d_shapes)
    )
)
def test_outer(x1, x2):
    ...
    assert res.dtype == dh.result_type(
        x1.dtype, x2.dtype
    ), "outer() did not return the correct dtype"

    if 0 in shape:
        true_res = _array_module.empty(shape, dtype=res.dtype)
    else:
        true_res = _array_module.asarray(
            [[x1[i] * x2[j] for j in range(x2.shape[0])] for i in range(x1.shape[0])],
            dtype=res.dtype,
        )
...
def test_qr(x, kw):
    ...
    if mode == "complete":
        assert Q.shape == x.shape[:-2] + (
            M,
            M,
        ), "qr().Q did not return the correct shape"
    else:
        assert Q.shape == x.shape[:-2] + (
            M,
            K,
    ...
    if mode == "complete":
        assert R.shape == x.shape[:-2] + (
            M,
            N,
        ), "qr().R did not return the correct shape"
    else:
        assert R.shape == x.shape[:-2] + (
            K,
            N,
...
def test_slogdet(x):
    ...
    assert (
        logabsdet.dtype == x.dtype
    ), "slogdet().logabsdet did not return the correct dtype"
    assert (
        logabsdet.shape == x.shape[:-2]
    ), "slogdet().logabsdet did not return the correct shape"

    _test_stacks(lambda x: linalg.slogdet(x).sign, x, res=sign, dims=0)
    _test_stacks(lambda x: linalg.slogdet(x).logabsdet, x, res=logabsdet, dims=0)
...
def solve_args():
    ...
    # with each other.
    x1 = shared(
        invertible_matrices(stack_shapes=stack_shapes.map(lambda pair: pair[0]))
    )
...
@pytest.mark.xp_extension("linalg")
@given(x=finite_matrices(), kw=kwargs(full_matrices=booleans()))
def test_svd(x, kw):
    res = linalg.svd(x, **kw)
    ...
    if full_matrices:
        ...
        assert U.shape == (
            *stack,
            M,
            K,
        ), "svd(full_matrices=False).U did not return the correct shape"
        assert Vh.shape == (
            *stack,
            K,
            N,
        ), "svd(full_matrices=False).Vh did not return the correct shape"
    assert S.shape == (*stack, K), "svd().S did not return the correct shape"

    # The values of s must be sorted from largest to smallest
    if K >= 1:
        assert _array_module.all(
            S[..., :-1] >= S[..., 1:]
        ), "svd().S values are not sorted from largest to smallest"
...
def test_svdvals(x):
    ...
    *stack, M, N = x.shape
    ...
    assert _array_module.all(
        res[..., :-1] >= res[..., 1:]
    ), "svdvals() values are not sorted from largest to smallest"
