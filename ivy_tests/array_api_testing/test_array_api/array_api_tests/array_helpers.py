...
__all__ = [
    ...
    "assert_positive_mathematical_sign",
    "negative_mathematical_sign",
    "assert_negative_mathematical_sign",
    "same_sign",
    "assert_same_sign",
    "float64",
    "asarray",
...
def assert_exactly_equal(x, y):
    ...
    assert (
        x.shape == y.shape
    ), f"The input arrays do not have the same shapes ({x.shape} != {y.shape})"

    assert (
        x.dtype == y.dtype
    ), f"The input arrays do not have the same dtype ({x.dtype} != {y.dtype})"
...
def isodd(x):
    return logical_and(
        isintegral(x),
        equal(remainder(x, 2 * one(x.shape, x.dtype)), one(x.shape, x.dtype)),
    )
...
def iseven(x):
    return logical_and(
        isintegral(x),
        equal(remainder(x, 2 * one(x.shape, x.dtype)), zero(x.shape, x.dtype)),
    )
...
def assert_positive_mathematical_sign(x):
    assert all(
        positive_mathematical_sign(x)
    ), "The input arrays do not have a positive mathematical sign"
...
def assert_negative_mathematical_sign(x):
    assert all(
        negative_mathematical_sign(x)
    ), "The input arrays do not have a negative mathematical sign"
