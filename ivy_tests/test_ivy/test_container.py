...
def test_container_scalar_and(device):
    ...
    assert np.allclose(ivy.to_numpy(container["a"], copy=False), np.array([True]))
    assert np.allclose(ivy.to_numpy(container.a, copy=False), np.array([True]))
    assert np.allclose(ivy.to_numpy(container["b"]["c"], copy=False), np.array([True]))
    assert np.allclose(ivy.to_numpy(container.b.c, copy=False), np.array([True]))
    assert np.allclose(ivy.to_numpy(container["b"]["d"], copy=False), np.array([False]))
    assert np.allclose(ivy.to_numpy(container.b.d, copy=False), np.array([False]))
...
def test_container_not(device):
    ...
    assert np.allclose(ivy.to_numpy(container["a"], copy=False), np.array([False]))
    assert np.allclose(ivy.to_numpy(container.a, copy=False), np.array([False]))
    assert np.allclose(ivy.to_numpy(container["b"]["c"], copy=False), np.array([False]))
    assert np.allclose(ivy.to_numpy(container.b.c, copy=False), np.array([False]))
    assert np.allclose(ivy.to_numpy(container["b"]["d"], copy=False), np.array([True]))
    assert np.allclose(ivy.to_numpy(container.b.d, copy=False), np.array([True]))
