...
@st.composite
def _random_tt_matrix_data(draw):
    n_stg = helpers.ints(min_value=2, max_value=6)
    n = draw(n_stg)
    while n % 2 != 0:
        n = draw(n_stg)
    shape = draw(
        st.lists(helpers.ints(min_value=1, max_value=5), min_size=n, max_size=n)
    )
    rank = draw(helpers.ints(min_value=1, max_value=len(shape)))
    dtype = draw(helpers.get_dtypes("float", full=False))
    full = draw(st.booleans())
...
@handle_test(
    fn_tree="functional.ivy.experimental.random_tt_matrix",
    data=_random_tt_matrix_data(),
    test_with_out=st.just(False),
    test_instance_method=st.just(False),
)
def test_random_tt_matrix(
    *,
    data,
    test_flags,
    backend_fw,
