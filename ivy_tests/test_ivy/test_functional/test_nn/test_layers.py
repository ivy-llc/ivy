...
@st.composite
def x_and_gru(draw, dtypes):
    dtype = draw(dtypes)

    seq_length = draw(helpers.ints(min_value=1, max_value=2))
    batch_size = draw(helpers.ints(min_value=1, max_value=2))
    hidden_size = draw(helpers.ints(min_value=1, max_value=2))
    input_size = draw(helpers.ints(min_value=1, max_value=2))
    hidden_size = draw(helpers.ints(min_value=1, max_value=2))

    x_shape = (seq_length, batch_size, input_size)
    w_shape = (3 * hidden_size, input_size)
    r_shape = (3 * hidden_size, hidden_size)
    b_shape = (6 * hidden_size,)
    ...
    w = draw(
        helpers.array_values(dtype=dtype[0], shape=w_shape, min_value=0, max_value=1)
    )
    r = draw(
        helpers.array_values(dtype=dtype[0], shape=r_shape, min_value=0, max_value=1)
    )
    b = draw(
        helpers.array_values(dtype=dtype[0], shape=b_shape, min_value=0, max_value=1)
    )
    initial_h = draw(
        helpers.array_values(
            dtype=dtype[0], shape=initial_h_shape, min_value=0, max_value=1
        )
    )
...
# gru
@handle_test(
    fn_tree="functional.ivy.experimental.gru",
    dtype_gru=x_and_gru(
        dtypes=helpers.get_dtypes("float"),
    ),
    test_with_out=st.just(False),
