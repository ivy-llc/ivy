...
@given(
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtype("numeric", full=True),
...
@given(
    dtype_and_start_stop=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtype("float", full=True),
        num_arrays=2,
        min_value=None,
        max_value=None,
        min_num_dims=1,
...
@given(
    ...
    dtype=helpers.get_dtype("integer"),
...
def _dtypes(draw):
    return draw(
        st.shared(
            helpers.list_of_length(x=helpers.get_dtype("numeric"), length=1),
            key="dtype",
        )
    )
...
dtype_shared = st.shared(helpers.get_dtype("numeric"), key="dtype")
