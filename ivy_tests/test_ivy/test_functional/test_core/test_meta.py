...
def test_maml_step_unique_vars(
    ...
    with update_backend(backend_fw) as ivy_backend:
        ...
        if batched:
            variables = ivy_backend.Container(
                {
                    "latent": variable_fn(
                        ivy.repeat(
                            ivy.array([[0.0]], device=on_device), num_tasks, axis=0
                        )
                    ),
                    "weight": variable_fn(
                        ivy.repeat(
                            ivy.array([[1.0]], device=on_device), num_tasks, axis=0
                        )
                    ),
                }
            )
            ...
            variables = ivy_backend.Container(
                {
                    "latent": variable_fn(ivy.array([0.0], device=on_device)),
                    "weight": variable_fn(ivy.array([1.0], device=on_device)),
                }
            )

        # batch
        batch = ivy_backend.Container(
            {"x": ivy.arange(1, num_tasks + 1, dtype="float32")}
        )
...
def test_maml_step_shared_vars(
    ...
    if batched:
        variables = ivy_backend.Container(
            {
                "latent": variable_fn(
                    ivy_backend.repeat(
                        ivy_backend.array([[1.0]], device=on_device), num_tasks, axis=0
                    )
                )
            }
        )
        ...
        variables = ivy_backend.Container(
            {"latent": variable_fn(ivy.array([1.0], device=on_device))}
        )

    # batch
    batch = ivy_backend.Container({"x": ivy.arange(1, num_tasks + 1, dtype="float32")})
    ...
    if stop_gradients:
        assert ivy_backend.equal(
            ivy_backend.functional.ivy._is_variable(calc_cost, exclusive=True), False
        )
    assert np.allclose(ivy_backend.to_scalar(calc_cost), true_cost)
    outer_grads = rets[1]
    assert np.allclose(
