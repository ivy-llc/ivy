...
@given(
    dtype_and_x=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypess("valid", full=True), num_arrays=1
    ),
    dtype=helpers.get_dtypess("valid"),
)
def test_astype(
    dtype_and_x,
    dtype,
...
@given(
    ...
    to_dtype=helpers.get_dtypess("valid"),
...
@handle_cmd_line_args
@given(
    input_dtype=helpers.get_dtypess("valid"),
...
@given(
    dtype_and_values=helpers.dtype_and_values(
        available_dtypes=helpers.get_dtypess("valid", full=True),
        num_arrays=2,
        shared_dtype=False,
    ),
    as_variable=helpers.list_of_length(x=st.booleans(), length=2),
