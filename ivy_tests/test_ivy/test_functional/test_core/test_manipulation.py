...
def _dtype_values_axis(draw, min_value=None, max_value=None):
    ...
    if min_value is None:
        min_axis = -len(shape)
    elif isinstance(min_value, types.FunctionType):  # noqa: F821
    ...
    elif isinstance(max_value, types.FunctionType):  # noqa: F821
...
def _array_dtype_reshape(draw):
    """
    Hypothesis strategy that will return an array, its dtype, 
    and a valid shape for it to be reshaped into
    """
    dtype, array, shape = draw(helpers.dtype_and_values(
        available_dtypes=ivy_np.valid_dtypes,
        ret_shape=True))
...
def _repeat_helper(draw):
    ...
    repeat_shape = (draw(st.one_of(st.just(1),
                                   st.just(shape[axis]))),
...
def _pad_helper(draw):
    ...
    pad_width = tuple(draw(st.lists(st.tuples(st.integers(0, 100), st.integers(0, 100)), 
...
def _split_helper(draw):
    noss_is_int = draw(st.shared(st.integers(1, 2), 
                                 key="noss_type").map(lambda x: x == 1))
