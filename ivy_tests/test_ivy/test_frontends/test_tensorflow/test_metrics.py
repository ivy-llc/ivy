...
@handle_cmd_line_args
@given(
    y_true=st.lists(st.integers(min_value=0, max_value=1), min_size=2, max_size=2),
    dtype_y_pred=helpers.dtype_and_values(
        min_value=0,
        max_value=1,
        shape=(2, 2),
        dtype=ivy_tf.valid_float_dtypes,
    ),
    num_positional_args=helpers.num_positional_args(
        fn_name="ivy.functional.frontends.tensorflow.sparse_categorical_accuracy"
    ),
    native_array=st.booleans(),
...
def test_sparse_categorical_accuracy(
    ...
    helpers.test_frontend_function(
        ...
        fw=fw,
        frontend="tensorflow",
        fn_tree="keras.metrics.sparse_categorical_accuracy",
        y_true=y_true,
        y_pred=y_pred,
    )
