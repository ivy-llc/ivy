...
@given(
    ...
    num_positional_args=helpers.num_positional_args(
        fn_name="ivy.functional.frontends.tensorflow.Atan2"
    ),
)
def test_tensorflow_Atan2(
    dtype_and_x, as_variable, with_out, num_positional_args, fw, native_array
):
    dtype, x = dtype_and_x
    helpers.test_frontend_function(
        input_dtypes=dtype,
        ...
        fw=fw,
        frontend="tensorflow",
        fn_tree="raw_ops.Atan2",
        x=np.asarray(x[1], dtype=dtype[1]),
        y=np.asarray(x[0], dtype=dtype[0]),
    )
