# global
from cupy import mgrid as cp_mgrid, ogrid as cp_ogrid
from hypothesis import given, strategies as st

# local
from ivy.functional.frontends.cupy import mgrid, ogrid
...
@given(
    ...
    num_positional_args=helpers.num_positional_args(
        fn_name="ivy.functional.frontends.cupy.arange"
    ),
)
def test_cupy_arange(
    start,
    stop,
    step,
    dtype,
...
@given(
    ...
    num_positional_args=helpers.num_positional_args(
        fn_name="ivy.functional.frontends.cupy.linspace"
    ),
)
def test_cupy_linspace(
    dtype_start_stop,
    num,
    axis,
    num_positional_args,
...
@given(
    ...
    axis=helpers.ints(min_value=-1, max_value=0),
    num_positional_args=helpers.num_positional_args(
        fn_name="ivy.functional.frontends.cupy.logspace"
    ),
)
def test_cupy_logspace(
    dtype_start_stop,
    num,
    base,
    axis,
...
def test_cupy_mgrid(
    ...
    if start and stop and step:
        ...
        ret_np = cp_mgrid[start:stop:step]
    elif start and step:
        ret = mgrid[start::step]
        ret_np = cp_mgrid[start::step]
    elif stop and step:
        ret = mgrid[:stop:step]
        ret_np = cp_mgrid[:stop:step]
    elif start and stop:
        ret = mgrid[start:stop]
        ret_np = cp_mgrid[start:stop]
    elif start:
        ret = mgrid[start:]
        ret_np = cp_mgrid[start:]
    else:
        ret = mgrid[:stop]
        ret_np = cp_mgrid[:stop]
...
def test_cupy_ogrid(
    ...
    if start and stop and step:
        ...
        ret_np = cp_ogrid[start:stop:step]
    elif start and step:
        ret = ogrid[start::step]
        ret_np = cp_ogrid[start::step]
    elif stop and step:
        ret = ogrid[:stop:step]
        ret_np = cp_ogrid[:stop:step]
    elif start and stop:
        ret = ogrid[start:stop]
        ret_np = cp_ogrid[start:stop]
    elif start:
        ret = ogrid[start:]
        ret_np = cp_ogrid[start:]
    else:
        ret = ogrid[:stop]
        ret_np = cp_ogrid[:stop]
    ret = helpers.flatten_and_to_np(ret=ret)
    ret_np = helpers.flatten_and_to_np(ret=ret_np)
    helpers.value_test(ret_np_flat=ret, ret_np_from_gt_flat=ret_np, rtol=1e-03)
