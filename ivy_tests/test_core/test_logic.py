...
# logical_and
@pytest.mark.parametrize("x1_n_x2", [([True, True], [False, True]), ([[0.]], [[1.]])])
...
def test_logical_and(x1_n_x2, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.logical_and, x1, x2),
        ivy.numpy.logical_and(ivy.to_numpy(x1), ivy.to_numpy(x2))
    )
...
def test_logical_or(x1_n_x2, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.logical_or, x1, x2),
        ivy.numpy.logical_or(ivy.to_numpy(x1), ivy.to_numpy(x2))
    )
...
# logical_not
@pytest.mark.parametrize("x", [[True, True], [[0.]]])
@pytest.mark.parametrize("dtype_str", ['float32'])
@pytest.mark.parametrize("tensor_fn", [ivy.array, helpers.var_fn])
