...
def test_stack_images():
    for lib, call in helpers.calls():
        if call in [helpers.tf_graph_call, helpers.mx_graph_call]:
            ...
            continue
        assert call(ivy.stack_images, [ivy.ones((1, 2, 3), f=lib)] * 4, (2, 1)).shape == (2, 4, 3)
        assert call(ivy.stack_images, [ivy.ones((8, 8, 3), f=lib)] * 9, (1, 1)).shape == (24, 24, 3)
        assert call(ivy.stack_images, [ivy.ones((3, 16, 12, 4), f=lib)] * 10, (2, 5)).shape == (3, 80, 36, 4)
        assert call(ivy.stack_images, [ivy.ones((5, 20, 9, 5), f=lib)] * 10, (5, 2)).shape == (5, 40, 72, 5)
        helpers.assert_compilable('stack_images', lib)
...
def test_bilinear_resample():
    for lib, call in helpers.calls():
        ...
        if call in [helpers.torch_call]:
            ...
            continue
        helpers.assert_compilable('bilinear_resample', lib)
...
def test_gradient_image():
    for lib, call in helpers.calls():
        ...
        assert np.array_equal(dx, np.array([[[[1.], [1.], [0.]],
                                             [[-1.], [-1.], [0.]],
                                             [[2.], [-1.], [0.]]]]))
        helpers.assert_compilable('gradient_image', lib)
