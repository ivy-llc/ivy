...
def test_variable(dev_str, call):
    if call is helpers.tf_graph_call:
        ...
        pytest.skip()
    call(ivy.variable, ivy.tensor([0.]))
    call(ivy.variable, ivy.tensor([0.], 'float32'))
    call(ivy.variable, ivy.tensor([[0.]]))
...
def test_execute_with_gradients(dev_str, call):
    ...
    xs = Container({'w': ivy.variable(ivy.tensor([3.]))})
    ...
    xs = Container({'w1': ivy.variable(ivy.tensor([3.])),
                    'w2': ivy.variable(ivy.tensor([5.]))})
    y, dydxs = call(ivy.execute_with_gradients, func, xs)
...
def test_gradient_descent_update(dev_str, call):
    ws = Container({'w': ivy.variable(ivy.tensor([3.]))})
    dcdws = Container({'w': ivy.tensor([6.])})
    w_new = ivy.tensor(ivy.gradient_descent_update(ws, dcdws, 0.1)['w'])
...
def test_adam_update(dev_str, call):
    ...
    w_new = ivy.tensor(ivy.adam_update(ws, dcdws, 0.1, mw, vw, ivy.tensor(1))[0]['w'])
