...
def test_execute_with_gradients(
        func_n_xs_n_ty_n_te_n_tg, dtype_str, tensor_fn, dev_str, call
):
    # smoke test
    ...
    if call is not helpers.mx_call:
        # mxnet cannot slice array down to shape (),
        # it remains fixed at size (1,)
...
def test_gradient_descent_update(
        ws_n_grads_n_lr_n_wsnew, dtype_str, tensor_fn, dev_str, call
):
    # smoke test
...
def test_adam_update(
    ...
    ws_new, mw_new, vw_new = ivy.adam_update(
        ws, dcdws, lr, mw, vw, ivy.array(1)
    )
...
def test_stop_gradient(x_raw, dtype_str, tensor_fn, dev_str, call):
    ...
    assert np.array_equal(
        call(ivy.stop_gradient, x), ivy.numpy.array(x_raw, dtype_str, dev_str)
    )
