...
def test_reduce_sum(x, axis, kd, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.reduce_sum, x), ivy.numpy.reduce_sum(ivy.to_numpy(x))
    )
...
def test_reduce_prod(x, axis, kd, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.reduce_prod, x), ivy.numpy.reduce_prod(ivy.to_numpy(x))
    )
...
def test_reduce_mean(x, axis, kd, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.reduce_mean, x), ivy.numpy.reduce_mean(ivy.to_numpy(x))
    )
...
def test_reduce_var(x, axis, kd, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.reduce_var, x), ivy.numpy.reduce_var(ivy.to_numpy(x))
    )
...
def test_reduce_min(x, axis, kd, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.reduce_min, x), ivy.numpy.reduce_min(ivy.to_numpy(x))
    )
...
def test_reduce_max(x, axis, kd, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.reduce_max, x), ivy.numpy.reduce_max(ivy.to_numpy(x))
    )
