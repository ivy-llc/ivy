...
# random_uniform
@pytest.mark.parametrize("low", [None, -1., 0.2])
@pytest.mark.parametrize("high", [None, 0.5, 2.])
...
def test_random_uniform(low, high, shape, dtype_str, tensor_fn, dev_str, call):
    ...
    kwargs = dict(
        [
            (k, tensor_fn(v))
            for k, v in zip(['low', 'high'], [low, high])
            if v is not None
        ]
    )
...
# multinomial
@pytest.mark.parametrize("probs", [[[1., 2.]], [[1., 0.5], [0.2, 0.3]], None])
@pytest.mark.parametrize("num_samples", [1, 2])
@pytest.mark.parametrize("replace", [True, False])
@pytest.mark.parametrize("dtype_str", ['float32'])
@pytest.mark.parametrize("tensor_fn", [ivy.array, helpers.var_fn])
def test_multinomial(
        probs, num_samples, replace, dtype_str, tensor_fn, dev_str, call
):
    population_size = 2
    if call in [
        helpers.mx_call, helpers.tf_call, helpers.tf_graph_call
    ] and not replace:
    ...
    # smoke test
    probs = tensor_fn(
        probs, dtype_str, dev_str
    ) if probs is not None else probs

    batch_size = probs.shape[0] if probs is not None else 2

    ret = ivy.multinomial(
        population_size, num_samples, batch_size, probs, replace
    )
...
def test_randint(low, high, shape, dtype_str, tensor_fn, dev_str, call):
    # smoke test
    if call in [
        helpers.mx_call, helpers.torch_call
    ] and tensor_fn is helpers.var_fn:
...
# shuffle
@pytest.mark.parametrize("x", [[1, 2, 3], [[1., 4.], [2., 5.], [3., 6.]]])
@pytest.mark.parametrize("dtype_str", ['float32'])
@pytest.mark.parametrize("tensor_fn", [ivy.array, helpers.var_fn])
