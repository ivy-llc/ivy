...
def test_leaky_relu(x, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.leaky_relu, x), ivy.numpy.leaky_relu(ivy.to_numpy(x))
    )
...
def test_gelu(x, approx, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.gelu, x, approx), ivy.numpy.gelu(ivy.to_numpy(x), approx)
    )
...
def test_sigmoid(x, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.sigmoid, x), ivy.numpy.sigmoid(ivy.to_numpy(x))
    )
...
def test_softmax(x, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.softmax, x), ivy.numpy.softmax(ivy.to_numpy(x))
    )
...
def test_softplus(x, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.softplus, x), ivy.numpy.softplus(ivy.to_numpy(x))
    )
