...
def test_cross_entropy(t_n_p_n_res, dtype_str, tensor_fn, dev_str, call):
    ...
    # value test
    assert np.allclose(
        call(ivy.cross_entropy, true, pred), np.asarray(true_target)
    )

    # compilation test
    if call in [helpers.torch_call]:
        # cross_entropy does not have backend implementation,
        # pytorch scripting requires direct bindings to work,
        # which bypass get_framework()
        return
...
@pytest.mark.parametrize(
    ...
    [
        (
            [[0., 1., 0.]],
            [[0.3, 0.7, 0.5]],
            [[0.35667494, 0.35667494, 0.69314718]]
        )
    ]
)
@pytest.mark.parametrize("dtype_str", ['float32'])
...
def test_binary_cross_entropy(
        t_n_p_n_res, dtype_str, tensor_fn, dev_str, call
):
    # smoke test
    true, pred, true_target = t_n_p_n_res
    pred = tensor_fn(pred, dtype_str, dev_str)
    ...
    if call in [helpers.torch_call]:
        ...
        # pytorch scripting requires direct bindings to work, which bypass
        # get_framework()
        return
