...
def test_logical_and():
    ...
    fname = os.path.join(this_file_dir, 'runtime_analysis/{}/logic/logical_and.txt'.format(DIM))
...
def test_logical_or():
    ...
    fname = os.path.join(this_file_dir, 'runtime_analysis/{}/logic/logical_or.txt'.format(DIM))
    if os.path.exists(fname):
        os.remove(fname)
    for lib, call in [(l, c) for l, c in helpers.calls if c not in [helpers.tf_graph_call, helpers.mx_graph_call]]:
...
def test_logical_not():
    ...
    fname = os.path.join(this_file_dir, 'runtime_analysis/{}/logic/logical_not.txt'.format(DIM))
    if os.path.exists(fname):
        os.remove(fname)
    for lib, call in [(l, c) for l, c in helpers.calls if c not in [helpers.tf_graph_call, helpers.mx_graph_call]]:
