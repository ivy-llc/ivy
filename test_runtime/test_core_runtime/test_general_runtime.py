"""
Collection of runtime tests for unified general functions
"""
...
def test_compile():
    ...
    fname = os.path.join(this_file_dir, 'runtime_analysis/{}/general/compile.txt'.format(DIM))
    if os.path.exists(fname):
        os.remove(fname)
    for lib, call in [(l, c) for l, c in helpers.calls if c not in [helpers.tf_graph_call, helpers.mx_graph_call]]:
        ...
        for _ in range(100):
            ...
            log_time(fname, 'tb0')
            ivy_gen_w_time.compile(some_fn, x0, f=time_lib)
            log_time(fname, 'tb4', time_at_start=True)

            log_time(fname, 'tt0')
            ivy_gen.compile(some_fn, x0, f=lib)
            log_time(fname, 'tt1', time_at_start=True)
