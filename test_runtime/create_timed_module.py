...
OFFSETS = {
    'general':
    {
        'jax': {
            ...
            'one_hot': [None],
            'identity': [-7],
            'scatter_flat': [-4, -5, -8, -9, -11, -12],
            'scatter_nd': [-4, -5, -8, -9, -11, -12],
            'gather_nd': [-3],
            'get_device': [-1]
        },
        'mxnd': {
        ...
        'torch': {
            ...
            'indices_where': [-2],
            'identity': [-7],
            'scatter_flat': [-2, -9, -12, -14, -16],
            'scatter_nd': [-4, -18, -21, -23, -25],
            'gather_nd': [-2],
            'get_device': [-1]
        }
    },
    ...
    'gradients': {
        'jax': {'execute_with_gradients': [-1, -10]},
        'mxnd': {
            'variable': [-1],
            'execute_with_gradients': [-1, -8],
            'gradient_descent_update': [None]
        },
        'numpy': {
            'variable': [None],
            'execute_with_gradients': [None],
            'stop_gradient': [None]
        },
        'tensorflow': {
            'execute_with_gradients': [-1, -8],
    ...
    'linalg': {
        'jax': {'vector_to_skew_symmetric_matrix': [None]},

        'mxnd': {
            'svd': [-1],
            'pinv': [None],
            'vector_to_skew_symmetric_matrix': [None]
        },
    ...
    for framework in ['jax', 'mxnd', 'numpy', 'tensorflow', 'torch']:
        for subdir, submodules in submodules_dict.items():
            for submodule in submodules:
                modify_ivy_file(
                    os.path.join(
                        with_time_logs_dir, 'ivy/{}/{}/{}.py'.format(
                            framework, subdir, submodule
                        )
                    ),
                    os.path.join(
                        this_file_dir, 'test_{}_runtime'.format(subdir)
                    ),
                ...
                with open(
                    os.path.join(
                        this_file_dir,
                        'test_{}_runtime/runtime_analysis/'
                        '{}/{}_offsets.json'.format(
                            subdir, dim, submodule
                        )
                    ), 'w+'
                ) as file:
