...
def test_conv1d():
    fname = os.path.join(
        this_file_dir,
        'runtime_analysis/{}/layers/conv1d.txt'.format(DIM)
    )
    ...
    for lib, call in [
        ...
        ivy_layers.conv1d(
            x0, filters, 1, "SAME",  data_format,
            filter_shape=[2], num_filters=1, f=lib
        )

        ivy_layers_w_time.conv1d(
            x0, filters, 1, "SAME", data_format,
            filter_shape=[2], num_filters=1, f=time_lib
        )
...
def test_conv1d_transpose():
    fname = os.path.join(
        this_file_dir,
        'runtime_analysis/{}/layers/conv1d_transpose.txt'.format(DIM)
    )
    ...
    for lib, call in [
        ...
        filters = ivy_gen.tensor(
            [[[0.]], [[1.]]], f=lib
        )

        ivy_layers.conv1d_transpose(
            x0, filters, 1, "SAME", (DIM, 2, 1), data_format,
            filter_shape=[2], num_filters=1, f=lib
        )

        ivy_layers_w_time.conv1d_transpose(
            x0, filters, 1, "SAME", (DIM, 2, 1), data_format,
            filter_shape=[2], num_filters=1, f=time_lib
        )
...
def test_conv2d():
    fname = os.path.join(
        this_file_dir,
        'runtime_analysis/{}/layers/conv2d.txt'.format(DIM)
    )
    ...
    for lib, call in [
        ...
        filters = ivy_gen.tensor([[[[0.]], [[1.]]], [[[1.]], [[0.]]]], f=lib)

        ivy_layers.conv2d(
            x0, filters, 1, "SAME", data_format,
            filter_shape=[2, 2], num_filters=1, f=lib
        )

        ivy_layers_w_time.conv2d(
            x0, filters, 1, "SAME", data_format,
            filter_shape=[2, 2], num_filters=1, f=time_lib
        )
...
def test_conv2d_transpose():
    fname = os.path.join(
        this_file_dir,
        'runtime_analysis/{}/layers/conv2d_transpose.txt'.format(DIM)
    )
    ...
    for lib, call in [
        ...
        ivy_layers.conv2d_transpose(
            x0, filters, 1, "SAME", (DIM, 2, 2, 1), data_format,
            filter_shape=[2, 2], num_filters=1, f=lib
        )

        ivy_layers_w_time.conv2d_transpose(
            x0, filters, 1, "SAME", (DIM, 2, 2, 1), data_format,
            filter_shape=[2, 2], num_filters=1, f=time_lib
        )
...
def test_depthwise_conv2d():
    fname = os.path.join(
        this_file_dir,
        'runtime_analysis/{}/layers/depthwise_conv2d.txt'.format(DIM)
    )
    ...
    for lib, call in [
        ...
        ivy_layers.depthwise_conv2d(
            x0, filters, 1, "SAME", data_format,
            filter_shape=[2, 2], num_filters=1, num_channels=1, f=lib
        )

        ivy_layers_w_time.depthwise_conv2d(
            x0, filters, 1, "SAME", data_format,
            filter_shape=[2, 2], num_filters=1, f=time_lib
        )
...
def test_conv3d():
    fname = os.path.join(
        this_file_dir,
        'runtime_analysis/{}/layers/conv3d.txt'.format(DIM)
    )
    ...
    for lib, call in [
        ...
        ivy_layers.conv3d(
            x0, filters, 1, "SAME", data_format,
            filter_shape=[2, 2, 2], num_filters=1, f=lib
        )

        ivy_layers_w_time.conv3d(
            x0, filters, 1, "SAME", data_format,
...
def test_conv3d_transpose():
    ...
    for lib, call in [
        ...
        ivy_layers.conv3d_transpose(
            x0, filters, 1, "SAME", (DIM, 2, 2, 2, 1), data_format,
            filter_shape=[2, 2, 2], num_filters=1, f=lib
        )

        ivy_layers_w_time.conv3d_transpose(
            x0, filters, 1, "SAME", (DIM, 2, 2, 2, 1), data_format,
            filter_shape=[2, 2, 2], num_filters=1, f=time_lib
        )
...
def test_linear():
    fname = os.path.join(
        this_file_dir, 'runtime_analysis/{}/layers/linear.txt'.format(DIM)
    )
