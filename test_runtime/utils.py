...
def append_to_file(f_name, string):
    with open(f_name, 'a+') as f:
        f.write('\n' + string + '\n')


def log_time(f_path, time_name, time_in=None, time_at_start=False):
    ...
    if time_in is not None:
        ...
        TIMES_DICT[f_path][time_name].append(count)
    else:
        TIMES_DICT[f_path][time_name].append(time.perf_counter())


def write_times():
    for f_path, f_path_dict in TIMES_DICT.items():
        time_names = f_path_dict.keys()
        time_lists = f_path_dict.values()
...
def modify_ivy_file(filepath, this_dir, offsets, namespace_dict, dim):
    ...
    all_methods = [
        line.split(def_key)[-1].split('(')[0]
        for i, line in enumerate(original_src_lines)
        if def_key in line
    ]

    all_def_indices = [
        i for i, line in enumerate(original_src_lines) if def_key in line
    ]
    ...
    log_dir = os.path.join(
        this_dir, 'runtime_analysis/{}/{}'.format(dim, module_name)
    )
    ...
    ivy_def_indices = [
        i for i, line in enumerate(new_src_lines)
        if def_key in line and def_key + '_' not in line
    ]
    ...
    for i, (method, def_idx) in enumerate(zip(ivy_methods, ivy_def_indices)):
        ...
        while True:
            ...
            if 'return ' in new_line or 'return\n' in new_line:
                ...
                new_src_lines.insert(
                    idx, spaces_str + 'log_time(fname, "tb3")\n'
                )
                ...
                if not found_a_return:
                    raise Exception(
                        'return should be found before white space.'
                    )
                ...
                if method in offsets[fw]:
                    for j, offset in enumerate(offsets[fw][method]):
                        ...
                        spaces_str = ' ' * (
                                len(new_src_lines[new_idx])
                                - len(new_src_lines[new_idx].lstrip(' '))
                        )

                        new_src_lines.insert(
                            new_idx, spaces_str + 'log_time(fname, "tb1")\n'
                        )

                        new_src_lines.insert(
                            new_idx + skip_val,
                            spaces_str
                            + 'log_time(fname, "tb2", time_at_start=True)\n'
                        )
    ...
    for i, line in enumerate(new_src_lines):
        ...
        overhead_line = (
            ...
            and '__dict__' not in line
            and ' return ' not in line
            and '    if ' not in line
            and 'log_time(fname, "tb1")' not in prev_line
            and line[0] == ' '
        )
        ...
        if overhead_line and not grouping:
            ...
            final_src_lines.insert(
                group_start_idx, spaces_str + 'log_time(fname, "to0")\n'
            )
            ...
            final_src_lines.insert(
                group_end_idx + 2,
                spaces_str + 'log_time(fname, "to1", time_at_start=True)\n'
            )
