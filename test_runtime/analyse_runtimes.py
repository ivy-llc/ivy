...
def main():
    ...
    core_log_dir = os.path.join(
        this_dir,
        'test_core_runtime/runtime_analysis/{}'.format(DIM)
    )

    core_submodule_dirs = [
        os.path.join(core_log_dir, submodule_dir)
        for submodule_dir in os.listdir(core_log_dir)
    ]

    nn_log_dir = os.path.join(
        this_dir, 'test_nn_runtime/runtime_analysis/{}'.format(DIM)
    )

    nn_submodule_dirs = [
        os.path.join(nn_log_dir, submodule_dir)
        for submodule_dir in os.listdir(nn_log_dir)
    ]

    submodule_dirs = core_submodule_dirs + nn_submodule_dirs

    filepaths = [
        os.path.join(submodule_dir, fname)
        for submodule_dir in submodule_dirs if submodule_dir[-5:] != '.json'
    ...
    reimplemented_dict = {
        'jax': {}, 'mxnd': {}, 'numpy': {}, 'tensorflow': {}, 'torch': {}
    }
    ...
    for json_filepath in json_filepaths:
        ...
        reimplemented_dict['jax'] = {
            **reimplemented_dict['jax'], **loaded_dict['jax']
        }

        reimplemented_dict['mxnd'] = {
            **reimplemented_dict['mxnd'], **loaded_dict['mxnd']
        }

        reimplemented_dict['numpy'] = {
            **reimplemented_dict['numpy'], **loaded_dict['numpy']
        }

        reimplemented_dict['tensorflow'] = {
            **reimplemented_dict['tensorflow'], **loaded_dict['tensorflow']
        }

        reimplemented_dict['torch'] = {
            **reimplemented_dict['torch'], **loaded_dict['torch']
        }
    ...
    for method, method_dict in all_results_dict.items():
        for f, f_dict in method_dict.items():
            ...
            if 'tt0' in f_dict[0] and 'tt1' in f_dict[0]:
                total_times_ = np.asarray(
                    [run['tt1'][0] - run['tt0'][0] for run in f_dict.values()]
                )

                total_times = _reject_outliers(total_times_)
                if len(total_times) == 0:
                    raise Exception(
                        'No data left after outlier rejection, '
            ...
            if 'tb1' in f_dict[0] and 'tb2' in f_dict[0]:
                backend_times_ = np.asarray(
                    [
                        np.sum(np.asarray(run['tb2']) - np.asarray(run['tb1']))
                ...
                backend_times_ = np.asarray(
                    [run['tb4'][0] - run['tb3'][0] for run in f_dict.values()]
                )
                ...
                raise Exception(
                    'Not enough backend times logged to compute runtimes.'
                )
            ...
            if 'to0' in f_dict[0] and 'to1' in f_dict[0]:
                overhead_times_ = np.asarray(
                    [
                        np.sum(np.asarray(run['to1']) - np.asarray(run['to0']))
                        for run in f_dict.values()
                    ]
                )
            ...
            if (
                method in reimplemented_dict[f]
                and reimplemented_dict[f][method] == [None]
            ):
            ...
            # total time
            total_mean_time = max(
                total_mean_time, backend_mean_time + overhead_mean_time
            )
            ...
            all_results_dict[method][f]['times']['graph_construct_time'] = max(
                total_mean_time - backend_mean_time - overhead_mean_time, 0
            )
        ...
        all_results_dict[method]['mean']['times']['total_time'] = np.mean(
            np.asarray(
                [
                    all_results_dict[method][fw]['times']['total_time']
                    for fw in FWS
        ...
        all_results_dict[method]['mean']['times']['backend_time'] = np.mean(
            np.asarray(
                [
                    all_results_dict[method][fw]['times']['backend_time']
                    for fw in FWS if fw in all_results_dict[method]
                ]
            )
        )

        all_results_dict[method]['mean']['times']['overhead_time'] = np.mean(
            np.asarray(
                [
                    all_results_dict[method][fw]['times']['overhead_time']
                    for fw in FWS if fw in all_results_dict[method]
                ]
            )
        )

        all_results_dict[method]['mean']['times']['graph_construct_time'] = np.mean(
            np.asarray(
                [
                    all_results_dict[method][fw]['times']['graph_construct_time']
                    for fw in FWS if fw in all_results_dict[method]
                ]
            )
        )
        ...
        all_results_dict[method]['mean']['ratios']['backend_ratio'] = np.mean(
            np.asarray(
                [
                    all_results_dict[method][fw]['ratios']['backend_ratio']
                    for fw in FWS if fw in all_results_dict[method]
                ]
            )
        )

        all_results_dict[method]['mean']['ratios']['ivy_overhead_ratio'] = np.mean(
            np.asarray(
                [
                    all_results_dict[method][fw]['ratios']['ivy_overhead_ratio']
                    for fw in FWS if fw in all_results_dict[method]
                ]
            )
        )

        all_results_dict[method]['mean']['ratios']['graph_construct_ratio'] = np.mean(
            np.asarray(
                [
                    all_results_dict[method][fw]['ratios']['graph_construct_ratio']
                    for fw in FWS if fw in all_results_dict[method]
                ]
            )
        )
    ...
    if os.path.exists(library_usage_fpath):
        ...
        for lib_name, lib_dict in usage_dict.items():
            ...
            for method_name, method_occurance in lib_dict.items():
                ...
                total_runtime += (
                    method_occurance * tot if not np.isnan(tot) else 0
                )

                back = all_results_dict[method_name]['mean']['times']['backend_time']
                backend_runtime += (
                    method_occurance * back if not np.isnan(back) else 0
                )

                over = all_results_dict[method_name]['mean']['times']['overhead_time']
                overhead_runtime += (
                    method_occurance * over if not np.isnan(over) else 0
                )

                graph = all_results_dict[method_name]['mean']['times']['graph_construct_time']
                graph_construct_runtime += (
                    method_occurance * graph if not np.isnan(graph) else 0
                )

            compiled_ratio = overhead_runtime/total_runtime
            eager_ratio = (
                overhead_runtime + graph_construct_runtime
            ) / total_runtime

            print(
                '\n{}:\neager_percentage: {}\ncompiled_percentage: {}\n'.format(
                    lib_name, eager_ratio*100, compiled_ratio*100)
            )
    ...
    for key_to_save in FWS + ['mean']:
        all_results_dict_ordered = collections.OrderedDict(
            sorted(
                all_results_dict.items(),
                key=lambda key_n_val: (
                    -key_n_val[1][key_to_save]['ratios']['ivy_overhead_ratio']
        ...
        with open(
                'csvs/{}/'.format(DIM) + key_to_save + '_runtime_analysis.csv',
                'w+'
        ) as file:
            ...
            for method, res_dict in all_results_dict_ordered.items():
                ...
                csv_writer.writerow(
                    [
                        method,
                        str(res_dict[key_to_save]['ratios']['ivy_overhead_ratio'] * 100),
