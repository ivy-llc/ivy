...
if __name__ == "__main__":
    ...
    with open("tests_to_run", "r") as f:
        for line in f:
            ...
            if with_gpu:
                ret = os.system(
                    f'docker run --rm --gpus all --env REDIS_URL={redis_url} --env REDIS_PASSWD={redis_pass} -v "$(pwd)":/ivy -v "$(pwd)"/.hypothesis:/.hypothesis unifyai/ivy:latest-gpu python3 -m pytest --tb=short {test} --device=all --backend={backend}'  # noqa
                    # noqa
                )
            else:
                ret = os.system(
