...
if __name__ == "__main__":
    ...
    with open("tests_to_run", "r") as f:
        for line in f:
            ...
            if with_gpu:
                ret = os.system(
                    f'docker run -it --rm --gpus all --env REDIS_URL={redis_url} --env REDIS_PASSWD={redis_pass} -v "$(pwd)":/ivy -v "$(pwd)"/.hypothesis:/.hypothesis unifyai/ivy:latest-gpu python3 -m pytest --device=gpu:0 --tb=short {test} --backend {backend}'  # noqa
                    # noqa
                )
            else:
                ret = os.system(
