...
if __name__ == "__main__":
    ...
    with open("tests_to_run", "r") as f:
        for line in f:
            ...
            if with_gpu:
                ret = os.system(
                    f'docker run --rm --gpus all -v "$(pwd)":/ivy -v "$(pwd)"/.hypothesis:/.hypothesis unifyai/multicuda:base_and_requirements python3 -m pytest --tb=short {test} --device=gpu:0 -B={backend}'  # noqa
                )
            else:
                ret = os.system(
                    f'docker run --rm --env -v "$(pwd)":/ivy -v "$(pwd)"/.hypothesis:/.hypothesis unifyai/ivy:latest python3 -m pytest --tb=short {test} --backend {backend}'  # noqa
                )
            if ret != 0:
                # res = make_clickable(run_id, result_config["failure"])
                failed = True
