...
def install_pkg(path,pkg,base='fw/'):
    if pkg.split('==')[0]=='torch':
        subprocess.run(f'pip3 install {pkg} --default-timeout=100 -f https://download.pytorch.org/whl/torch_stable.html --target={path}', shell=True)
    elif pkg.split('==')[0]=='jaxlib':
        subprocess.run(f'pip3 install {pkg} --default-timeout=100 -f https://storage.googleapis.com/jax-releases/jax_releases.html  --target={path}',shell=True)
    else:
        subprocess.run(f'pip3 install {pkg} --default-timeout=100  --target={path}',shell=True)

torch_req=['torch/1.4.0','torch/1.5.0','torch/1.10.1']
tensorflow_req=['tensorflow/2.2.0','tensorflow/2.2.1','tensorflow/2.2.2','tensorflow/2.4.4','tensorflow/2.9.0','tensorflow/2.9.1']
