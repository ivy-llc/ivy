...
class Process_issue():
    def __init__(self, issue_number, comment_number, comment_body, comment_author, token):
        self._token = token
        self._issue_number = issue_number
        self._comment_body = comment_body
        self._comment_author = comment_author
        self._comment_number = comment_number
        self._issue = self.command(f'gh issue view {self._issue_number} --json title,body,labels')
    ...
    def main_issue_body_functions(self, allocated=True):
        ...
        return [i.strip() for i in re.findall(r"\s\w+", self._issue['body'])]
