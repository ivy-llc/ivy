...
def main():
    ...
    main_issue = Process_issue(int(sys.argv[1]), int(sys.argv[2]), sys.argv[3], sys.argv[4], sys.argv[-1])
    ...
    if targeted_labels:
        if issue_in_cmt:
            ...
            if comment_issue_title in non_alocate_functions:
                ...
                main_issue.command(f'gh issue comment {comment_issue_id} --body "This issue is being closed because the function has already been taken, please choose another function and recommend on the main issue."', save_output=False)
                main_issue.command(f'gh issue close {comment_issue_id}', save_output=False)
                main_issue.delete_comment()
