...
* We pass :code:`fn_name` to :code:`helpers.num_positional_args` which is used to determine the number of positional arguments for :code:`ivy.functional.frontends.jax.lax.tan`.
* Note how we do not generate any values for :code:`fw` these are generated by pytest and are only passed as an arguement to :code:`test_jax_lax_tan()`.
...
* We cast :code:`x` as a numpy array with :code:`np.asarray(x, dtype=input_dtype)` whenever the frontend function requires an array as an input. This is becuase :code:`x` is generated as a :code:`list`
* One last important note is that all helper functions are designed to take keyword arguements.
