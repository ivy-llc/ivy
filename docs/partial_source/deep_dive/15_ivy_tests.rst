...
.. _`parametrize`: https://github.com/unifyai/ivy/blob/3dda99b1ed295e0ac67de7c4170dd041c23b78bc/ivy_tests/test_ivy/test_functional/test_core/test_general.py#L928
...
and mixed. Exhaustive data generation implies that all possible inputs and combinations of inputs are covered. However,
such rigorous testing is not necessary for standalone functions (those that are entirely self-contained in the Ivy
codebase without external references). These kinds of functions may only require standard Pytest testing using
`parametrize`_. For further clarity on the various function types in ivy, see `here`_.
