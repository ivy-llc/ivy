...
.. code-block:: python
    ...
    >>>     for i in range:(n)
    >>>         print("Example:", custom_strategy.example())
...
a 1-D array or a scaler value, which also generate an index if an array is generated, otherwise None.
...
`data`_ strategy, all of the helpers are implemented in as a `composite` strategy, this provides reusability of the
