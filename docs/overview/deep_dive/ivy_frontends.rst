...
When transpiling between frameworks, the first step is to trace the computation graph into low level python functions for the source framework using Ivy's tracer, before then replacing these nodes with the associated functions in Ivy's frontend API.
...
We wish to enable varying extents of dynamicism when compiling a graph with our tracer, and therefore we also implement the functions in the :mod:`jax.numpy` namespace in our frontend API for JAX.
