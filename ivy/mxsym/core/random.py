...
def random_uniform(low=0., high=1., shape=None, dev='cpu'):
    ...
    return _mx.symbol.random.uniform(low, high, shape, ctx=ctx)


def randint(low, high, shape, dev='cpu'):
    ctx = _mxnet_init_context(dev)
    return _mx.symbol.random.randint(low, high, shape, ctx=ctx)


seed = lambda seed_value: _mx.random.seed(seed_value)
shuffle = lambda x: _mx.symbol.random.shuffle(x)
