...
def bilinear_resample(x, warp):
    ...
    mat_flat = _mx.nd.reshape(
        x, [batch_shape_product] + input_image_dims + [-1]
    )
    ...
    interpolated_flat = _mx.nd.transpose(
        interpolated_flat_transposed, (0, 2, 3, 1)
    )
...
def gradient_image(x):
    ...
    dy = _ivy.concatenate(
        (
            dy, _ivy.zeros(
                batch_shape + [1, image_dims[1], num_dims], dev_str=dev_str
            )
        ), -3
    )
    dx = _ivy.concatenate(
        (
            dx, _ivy.zeros(
                batch_shape + [image_dims[0], 1, num_dims], dev_str=dev_str
            )
        ), -2
    )
