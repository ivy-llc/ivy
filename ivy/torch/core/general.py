...
def array(
    ...
    if dtype_str is not None:
        return (
            torch.tensor(
                object_in, dtype=dtype_from_str(dtype_str)
            ).to(str_to_dev(dev_str))
        )
...
# noinspection PyShadowingNames
def arange(
    stop: Number, start: Number = 0, step: Number = 1,
    dtype_str: Optional[str] = None, dev_str: Optional[str] = None
):
    if dtype_str is not None:
        return torch.arange(
            start, stop, step=step, dtype=dtype_from_str(dtype_str)
        ).to(str_to_dev(dev_str))
...
def _differentiable_linspace(start, stop, num, device):
    ...
    increments = (
        increment_tiled * torch.linspace(1, n_m_1, n_m_1, device=device)
    )
...
def linspace(start, stop, num, axis=None, dev_str=None):
    ...
    if stop_is_array:
        ...
        if num == 1:
            return (
                torch.ones(stop_shape[:axis] + [1] + stop_shape[axis:]) * start
            ).to(str_to_dev(dev_str))
    ...
    if start_is_array and stop_is_array:
        if num < start.shape[0]:
            ...
            res = [
                linspace_method(
                    strt, stp, num, device=str_to_dev(dev_str)
        ...
        if num < start.shape[0]:
            ...
            res = [
                linspace_method(
                    strt, stop, num, device=str_to_dev(dev_str)
                ) for strt in start
            ]
        ...
        if num < stop.shape[0]:
            ...
            res = [
                linspace_method(
                    start, stp, num, device=str_to_dev(dev_str)
                ) for stp in stop
            ]
...
def flip(
    ...
    num_dims: int = (
        len(batch_shape) if batch_shape is not None else len(x.shape)
    )
    ...
    if num_or_size_splits is None:
        ...
        if with_remainder:
            ...
            if remainder == 0:
                num_or_size_splits = (
                    round(
                        torch.tensor(dim_size) / torch.tensor(
                            num_or_size_splits
                        )
                    )
                )
            ...
            num_or_size_splits = (
                round(torch.tensor(dim_size) / torch.tensor(num_or_size_splits))
            )
...
def constant_pad(x, pad_width: List[List[int]], value: Number = 0.):
    ...
    return torch.nn.functional.pad(
        x, pad_width_flat, mode='constant', value=value
    )
...
def zeros(
    ...
    type_dict: Dict[str, torch.dtype] = {
        'bool': torch.bool,
        'int8': torch.int8,
        'uint8': torch.uint8,
        'int16': torch.int16,
...
def one_hot(indices, depth: int, dev_str: Optional[str] = None):
    ...
    return torch.nn.functional.one_hot(
        indices.type(torch.int64), depth
    ).to(str_to_dev(dev_str))
...
def scatter_flat(
    ...
    output = torch.ones(
        [size], dtype=dtype
    ).to(str_to_dev(dev_str)) * initial_val
    ...
    if torch_scatter is None:
        try:
            ...
            raise Exception(
                'Unable to import torch_scatter,'
    ...
    res = torch.where(
        res == initial_val,
        torch.zeros([size], dtype=updates.dtype).to(str_to_dev(dev_str)),
        res
    )
...
def scatter_nd(indices, updates, shape, reduction='sum', dev_str=None):
    ...
    flat_output = torch.ones(
        flat_result_size, dtype=dtype
    ).to(str_to_dev(dev_str)) * initial_val

    flat_updates = torch.reshape(updates, (-1,))
    new_shape = [1] * (len(indices_shape) - 1) + [num_index_dims]
    indices_scales = torch.reshape(
        result_dim_sizes[0:num_index_dims], new_shape
    )
    ...
    implicit_indices = torch.unsqueeze(
        torch.arange(implicit_indices_factor).to(str_to_dev(dev_str)), 0
    ).repeat(*[indices_for_flat_tiled.shape[0], 1])

    indices_for_flat = indices_for_flat_tiled + implicit_indices
    flat_indices_for_flat = torch.reshape(
        indices_for_flat, (-1,)
    ).type(torch.long)
    ...
    flat_scatter = torch_scatter.scatter(
        flat_updates, flat_indices_for_flat, out=flat_output, reduce=reduction
    )
    ...
    flat_scatter = torch.where(
        ...
        torch.zeros(
            flat_result_size, dtype=updates.dtype
...
def gather(params, indices, axis=-1, dev_str: Optional[str] = None):
    ...
    return torch.gather(
        params, axis, indices.type(torch.int64)
    ).to(str_to_dev(dev_str))
