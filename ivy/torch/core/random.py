...
def random_uniform(
    ...
    shape: Optional[List[int]] = None, dev_str: str = 'cpu'
):
    rand_range = high - low
    ...
    return torch.rand(true_shape).to(
        dev_str.replace('gpu', 'cuda')
    ) * rand_range + low


def multinomial(
        population_size: int, num_samples: int, batch_size: int,
    ...
    return torch.multinomial(
        probs, num_samples, replace
    ).to(dev_str.replace('gpu', 'cuda'))
