...
def execute_with_gradients(func, xs, retain_grads=False):
    ...
    if isinstance(func_ret, tuple):
        ...
        rest = tuple()
    x_grads_flat = list(
        _torch.autograd.grad([y], [v for k, v in xs.to_iterator()],
        retain_graph=retain_grads, create_graph=retain_grads)
    )
...
def _adam_update_trackable(ws, dcdws, alpha, mw, vw, epsilon):
    ws = ws.map(
        lambda w, key_chain: (
            w - alpha * mw.at_key_chain(key_chain) / (
...
def _adam_update_inplace(ws, dcdws, alpha, mw, vw, epsilon):
    ...
    def _inplace_update(x, key_chain):
        x.data -= (
            alpha * mw.at_key_chain(key_chain) / (
                vw.at_key_chain(key_chain) ** 0.5 + epsilon
            )
        )
