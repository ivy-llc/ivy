...
class ivy_Sequential(ivy.Module):
    _modules: typing.Any
    ...
    def __add__(self, other):
        if isinstance(other, ivy_Sequential):
            ret = ivy_Sequential()
            for layer in self:
                ret.append(layer)
            for layer in other:
    ...
    def insert(self, index, module):
        if not isinstance(module, ivy.Module):
            raise AssertionError(f"module should be of type: {ivy.Module}")
        n = len(self._modules)
        if not -n <= index <= n:
