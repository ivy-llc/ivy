...
def tensorflow__get_size_bknd(scale_factor, size, dims, input_shape):
    if scale_factor is not None:
        ...
        size = tuple(
            int(
                math.floor(
                    tensorflow_get_item(input_shape, i)
                    * tensorflow_get_item(scale_factor, i)
                )
            )
            for i in range(dims)
        )
        ...
        scale_factor = [
            tensorflow_divide(
                tensorflow_get_item(size, i), tensorflow_get_item(input_shape, i)
            )
            for i in range(dims)
        ]
