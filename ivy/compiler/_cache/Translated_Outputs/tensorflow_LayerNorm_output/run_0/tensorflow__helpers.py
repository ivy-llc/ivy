...
def tensorflow_equal(
    ...
    *,
    ...
    try:
        ...
        x1 = oirg_x1
        x2 = oirg_x2
    return tensorflow.math.equal(x1, x2)
...
def tensorflow_var(
    ...
    *,
    ...
    if size - correction <= 0:
        ret = tensorflow.math.reduce_variance(x, axis=axis, keepdims=keepdims)
        ret = tensorflow.cast(tensorflow.fill(ret.shape, float("nan")), ret.dtype)
        return ret
    else:
        return (
            tensorflow.math.reduce_variance(x, axis=axis, keepdims=keepdims)
            * size
            / (size - correction)
        )
...
def tensorflow_layer_norm_frnt(
    input, normalized_shape, weight=None, bias=None, eps=1e-05
):
    shape = tensorflow_shape(input)
    if isinstance(normalized_shape, int) and normalized_shape == shape[-1]:
        axis = [-1]
    else:
        assert tensorflow_all(
            tensorflow_equal(
                normalized_shape,
                tensorflow_get_item(shape, slice(-len(normalized_shape), None, None)),
            )
        )
        axis = list(range(len(shape) - len(normalized_shape), len(shape)))
    return tensorflow_layer_norm_bknd(input, axis, scale=weight, offset=bias, eps=eps)
