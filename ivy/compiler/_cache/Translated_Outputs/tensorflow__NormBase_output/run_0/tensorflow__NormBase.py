...
class tensorflow__NormBase(tensorflow_keras_Model):
    _version = 2
    ...
    def __init__(
        ...
        if self.affine:
            self.weight = tensorflow.Variable(
                tensorflow_empty_1(num_features, **factory_kwargs),
                name="tensorflow__NormBase/weight",
            )
            self.bias = tensorflow.Variable(
                tensorflow_empty_1(num_features, **factory_kwargs),
                name="tensorflow__NormBase/bias",
            )
        else:
        ...
        if self.track_running_stats:
            self.register_buffer(
                "running_mean", tensorflow_zeros_1(num_features, **factory_kwargs)
            )
            self.register_buffer(
                "running_var", tensorflow_ones_1(num_features, **factory_kwargs)
            )
    ...
    def _load_from_state_dict(
        ...
        if (version is None or version < 2) and self.track_running_stats:
            ...
            if num_batches_tracked_key not in state_dict:
                with tensorflow.name_scope("tensorflow__NormBase/state_dict"):
    ...
    def __repr__(self):
        ...
        if extra_repr:
            with tensorflow.name_scope("tensorflow__NormBase/extra_lines"):
    ...
    def _create_variables(self, device=None, dtype=None):
        with tensorflow.name_scope("tensorflow__NormBase/v"):
            v = dict(
                OrderedDict(
                    [
                        (k.replace(".", "/"), v)
    ...
    def _replace_update_v(self, new_v, native=None):
        with tensorflow.name_scope("tensorflow__NormBase/native"):
