    ...
    def call(self, input):
        ...
        if self.training and self.track_running_stats:
            if self.num_batches_tracked is not None:
                with tensorflow.name_scope("tensorflow__BatchNorm/num_batches_tracked"):
        ...
        with tensorflow.name_scope(
            "tensorflow__BatchNorm/running_mean, selfrunning_var)"
        ):
            normalized, self.running_mean, self.running_var = tensorflow_batch_norm_1(
                input,
                (
                    self.running_mean
