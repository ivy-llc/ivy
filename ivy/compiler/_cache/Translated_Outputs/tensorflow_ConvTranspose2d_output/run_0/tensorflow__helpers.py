...
def tensorflow_size_frnt_(arr, dim=None):
    ...
    if dim is None:
        return shape
    try:
        return tensorflow_get_item(shape, dim)
...
def tensorflow__calculate_fan_in_and_fan_out(tensor):
    dimensions = tensorflow_dim_frnt_(tensor)
    if dimensions < 2:
        raise ValueError(
            "Fan in and fan out can not be computed for tensor with fewer than 2 dimensions"
        )
    num_input_fmaps = tensorflow_size_frnt_(tensor, 1)
    num_output_fmaps = tensorflow_size_frnt_(tensor, 0)
    receptive_field_size = 1
    if tensorflow_dim_frnt_(tensor) > 2:
...
def tensorflow__check_bounds_and_get_shape_bknd(low, high, shape):
...
def tensorflow_kaiming_uniform_(
    ...
    return tensorflow_uniform__frnt_(tensor, -bound, bound, generator=generator)


def tensorflow__no_grad_uniform_(tensor, a, b, generator=None):
    return tensorflow_uniform__frnt_(tensor, a, b, generator=generator)


def tensorflow_uniform_(tensor, a=0.0, b=1.0, generator=None):
...
def tensorflow_current_backend_str():
    return "tensorflow"


def tensorflow__deconv_length_bknd(
    dim_size, stride_size, kernel_size, padding, dilation=1
):
...
def tensorflow__transpose_out_pad(
    ...
    if isinstance(padding, str):
        out_shape = [
            tensorflow__deconv_length_bknd(
...
def tensorflow_conv1d_transpose(
    x: Union[tensorflow.Tensor, tensorflow.Variable],
    ...
    /,
    *,
    output_shape: Optional[Union[tf.TensorShape, Sequence[int]]] = None,
    ...
    permuted_x = False
    if data_format == "NCW" and tensorflow_dev(x) == "cpu":
        x = tensorflow.transpose(x, (0, 2, 1))
        data_format = "NWC"
        permuted_x = True
    if filter_format == "channel_first":
        filters = tensorflow.transpose(filters, (2, 1, 0))
    output_shape, padding = tensorflow__transpose_out_pad(
        x.shape, filters.shape, strides, padding, 1, dilations, data_format
    )
    res = tensorflow.nn.conv1d_transpose(
        x, filters, output_shape, strides, padding, data_format, dilations
    )
    if bias is not None:
        if data_format[1] == "C":
            bias = tensorflow.reshape(bias, [1, -1, 1])
        res = tensorflow.math.add(res, bias)
    if permuted_x:
        res = tensorflow.transpose(res, (0, 2, 1))
    return res


def tensorflow_conv2d_transpose(
    x: Union[tensorflow.Tensor, tensorflow.Variable],
    filters: Union[tensorflow.Tensor, tensorflow.Variable],
    ...
    *,
    ...
    if data_format == "NCHW" and tensorflow_dev(x) == "cpu":
        ...
        permuted_x = True
    if filter_format == "channel_first":
        filters = tensorflow.transpose(filters, (2, 3, 1, 0))
    output_shape, padding = tensorflow__transpose_out_pad(
        x.shape, filters.shape, strides, padding, 2, dilations, data_format
    )
    res = tensorflow.nn.conv2d_transpose(
        x, filters, output_shape, strides, padding, data_format, dilations
    )
    if bias is not None:
        if data_format[1] == "C":
            bias = tensorflow.reshape(bias, [1, -1, 1, 1])
        res = tensorflow.math.add(res, bias)
    if permuted_x:
...
def tensorflow_conv3d_transpose(
    x: tf.Tensor,
    filters: tf.Tensor,
    ...
    *,
    ...
    permuted_x = False
    if data_format == "NCDHW" and tensorflow_dev(x) == "cpu":
        x = tensorflow.transpose(x, (0, 2, 3, 4, 1))
        data_format = "NDHWC"
        permuted_x = True
    if filter_format == "channel_first":
        filters = tensorflow.transpose(filters, (2, 3, 4, 1, 0))
    output_shape, padding = tensorflow__transpose_out_pad(
    ...
    res = tensorflow.nn.conv3d_transpose(
        x, filters, output_shape, strides, padding, data_format, dilations
    )
    if bias is not None:
        if data_format[1] == "C":
            bias = tensorflow.reshape(bias, [1, -1, 1, 1, 1])
        res = tensorflow.math.add(res, bias)
    if permuted_x:
        return tensorflow.transpose(res, (0, 4, 1, 2, 3))
    return res


def tensorflow__get_x_data_format_bknd(
    dims: int = 2, data_format: str = "channel_first"
):
    if dims == 1:
...
@tensorflow_handle_array_like_without_promotion
def tensorflow_conv_general_transpose(
    x: Union[tensorflow.Tensor, tensorflow.Variable],
    filters: Union[tensorflow.Tensor, tensorflow.Variable],
    strides: Union[int, Tuple[int, int]],
    padding: str,
    ...
    *,
    ...
    if feature_group_count == 1:
        if dims == 1:
            return tensorflow_conv1d_transpose(
                x,
                filters,
                strides,
                padding,
            ...
            return tensorflow_conv2d_transpose(
                x,
                filters,
                strides,
                padding,
            ...
            return tensorflow_conv3d_transpose(
                x,
                filters,
                strides,
                padding,
        ...
        permuted_x = False
        if data_format == "channel_first" and tensorflow_dev(x) == "cpu":
        ...
        res = tensorflow.math.add(res, bias) if bias is not None else res
        if permuted_x:
            return tensorflow.transpose(res, (0, dims + 1, *range(1, dims + 1)))
        return res


def tensorflow__get_transpose_pad_frnt(padding, output_padding, dims):
    padding, output_padding = map(
        lambda x: [x] * dims if isinstance(x, int) else x, [padding, output_padding]
    )
    ag__result_list_0 = []
    for pad, output_pad in zip(padding, output_padding):
        res = [pad, pad - output_pad]
        ag__result_list_0.append(res)
    asymmetric_padding = ag__result_list_0
...
def tensorflow_flip(
    ...
    *,
    ...
    if not num_dims:
        ...
        ret = tensorflow.reverse(x, new_axis)
    return ret
...
def tensorflow__conv_transpose_frnt(
    input,
    weight,
    bias=None,
    stride=1,
    ...
    weight = tensorflow_permute_dims(weight, axes=(*range(2, dims + 2), 0, 1))
    for i in range(dims):
        weight = tensorflow_flip(weight, axis=i)
    ...
    for i in range(dims):
        res = (
            (tensorflow_get_item(weight.shape, i) - 1)
            * tensorflow_get_item(dilation, i)
            + max(
                [
                    tensorflow_get_item(output_padding, i)
        ...
        unpad_slice = unpad_slice + (
            slice(
                max(
                    [
                        tensorflow_get_item(padding, i)
                        - tensorflow_get_item(dilation, i) // 2,
                        tensorflow_get_item(padding, i),
                        tensorflow_get_item(output_padding, i),
                    ]
                ),
                tensorflow_get_item(ret.shape, 2 + i)
                - tensorflow_get_item(padding, i)
                + tensorflow_get_item(output_padding, i)
                + tensorflow_get_item(dilation, i) // 2,
                1,
    ...
    ret = tensorflow_get_item(ret, unpad_slice)
    return ret


def tensorflow_conv_transpose2d_frnt(
    input,
    weight,
    bias=None,
    stride=1,
    ...
    if tensorflow_current_backend_str() in ["torch", "tensorflow"]:
        return tensorflow_conv_general_transpose(
            input,
            weight,
            stride,
            tensorflow__get_transpose_pad_frnt(padding, output_padding, 2),
            dims=2,
        ...
        return tensorflow__conv_transpose_frnt(
            input,
            weight,
            bias=bias,
            stride=stride,
