...
class tensorflow__NormBase(tensorflow_keras_Layer):
    ...
    def reset_running_stats(arr):
        if arr.track_running_stats:
            tensorflow_zero_(arr.running_mean)
            tensorflow_fill_(arr.running_var, 1)
            tensorflow_zero_(arr.num_batches_tracked)
            ...
            if num_batches_tracked_key not in state_dict:
                with tensorflow.name_scope("state_dict"):
                    state_dict = tensorflow_set_item(
                        ...
                        and arr.num_batches_tracked.device != tensorflow_device("meta")
    ...
    def _build(arr, *args, **kwargs):
        for module in arr.__dict__.values():
            if isinstance(module, tensorflow_keras_Layer) and module is not arr:
