...
from .tensorflow__helpers import tensorflow_split_3
from .tensorflow__helpers import tensorflow_uniform__1


class tensorflow__ConvNd(tensorflow_keras_Model):
    __constants__ = [
        "stride",
        "padding",
        "dilation",
    ...
    def __init__(
            ...
            if padding == "same":
                for d, k, i in zip(
                    ...
                    with tensorflow.name_scope(
                        "tensorflow__ConvNd/_reversed_padding_repeated_twice"
                    ):
        ...
        if transposed:
            self.weight = tensorflow.Variable(
                ...
                name="tensorflow__ConvNd/weight",
            )
        else:
            self.weight = tensorflow.Variable(
                tensorflow_empty_1(
        ...
        if bias:
            self.bias = tensorflow.Variable(
                tensorflow_empty_1(out_channels, **factory_kwargs),
                name="tensorflow__ConvNd/bias",
            )
        else:
    ...
    def reset_parameters(self):
        ...
        if self.bias is not None:
            with tensorflow.name_scope("tensorflow__ConvNd/"):
    ...
    def __repr__(self):
        ...
        if extra_repr:
            with tensorflow.name_scope("tensorflow__ConvNd/extra_lines"):
                extra_lines = tensorflow_split_3(extra_repr, "\n")
        child_lines = []
    ...
    def _create_variables(self, device=None, dtype=None):
        with tensorflow.name_scope("tensorflow__ConvNd/v"):
            v = dict(
                OrderedDict(
                    [
                        (k.replace(".", "/"), v)
        ...
        for module_prefix, module in modules:
            ...
            for k, v in members:
                if v is None or id(v) in memo:
                    continue
                if remove_duplicate:
                    tensorflow_add_2(memo, id(v))
                name = module_prefix + ("." if module_prefix else "") + k
                yield name, v

    def _replace_update_v(self, new_v, native=None):
        with tensorflow.name_scope("tensorflow__ConvNd/native"):
    ...
    def get_submodule(self, target):
        if target == "":
            return self
        atoms: typing.Any = tensorflow_split_3(target, ".")
        mod: typing.Any = self
        for item in atoms:
    ...
    def named_children(self):
        ...
        for name, module in self._module_dict.items():
            if module is not None and id(module) not in memo:
                tensorflow_add_2(memo, id(module))
                yield name, module
