    ...
    *,
    ...
    if training:
        ...
        if data_format == "NCHW":
            ...
            x = tensorflow.transpose(x, perm)
        res = tensorflow.nn.dropout(x, prob)
        if data_format == "NCHW":
            perm = (0, 3, 1, 2) if is_batched else (2, 0, 1)
            res = tensorflow.transpose(res, perm)
    else:
        res = x
    return res


def tensorflow_dropout2d_frnt(input, p=0.5, training=True, inplace=False):
    if tensorflow_ndim_frnt_(input) < 2:
        raise ValueError("Feature dropout requires at least 2 dimensions in the input")
    ret = tensorflow_dropout2d(input, p, training=training, data_format="NHWC")
    if inplace:
        tensorflow_inplace_update(input, ret)
        return input
    return ret
