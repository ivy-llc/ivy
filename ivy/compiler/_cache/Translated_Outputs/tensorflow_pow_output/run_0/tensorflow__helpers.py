...
def tensorflow_isinf(
    ...
    *,
    ...
    if not tensorflow_is_complex_dtype_bknd(x):
        if tensorflow_is_int_dtype_bknd(x):
            return tensorflow.zeros_like(x, tensorflow.bool)
        elif detect_negative and detect_positive:
            return tensorflow.math.is_inf(x)
        elif detect_negative:
            return tensorflow.experimental.numpy.isneginf(x)
        elif detect_positive:
            return tensorflow.experimental.numpy.isposinf(x)
    return tensorflow.zeros_like(x, tensorflow.bool)
