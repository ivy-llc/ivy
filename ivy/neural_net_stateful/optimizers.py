...
class Optimizer(abc.ABC):
    ...
    def __init__(self, lr, dev_str, compile_step=False):
        """
        Construct an general Optimizer.
        This is an abstract class, and must be derived.

        :param lr: Learning rate.
        :type lr: function or float.
        :param compile_step: Whether to compile the optimizer step,
            default is False.
    ...
    def _step(self, v, grads):
        """
        Update nested variables container v from update step,
    ...
    def step(self, v, grads):
        """
        Update nested variables container v from possibly compiled
            overriden private self._step_fn
...
class SGD(Optimizer):
    ...
    def _step(self, v, grads):
        ...
        :param grads: Nested gradients to update.
        :type grads: sequence of arrays
        :return: The new updated variables container,
            following gradient descent step.
        """
        new_v = ivy.gradient_descent_update(
            v, grads, self._lr if isinstance(self._lr, float) else self._lr()
        )
...
class Adam(Optimizer):
    ...
    def __init__(
        self, lr=1e-4, beta1=0.9, beta2=0.999, epsilon=1e-07,
            compile_step=False, dev_str=None
    ):
        """
