...
class Constant:
    ...
    def create_variables(self, var_shape, dev_str, fan_out=None, fan_in=None):
        ...
        """
        return ivy.variable(
            ivy.ones(var_shape, dev_str=dev_str) * self._constant
        )
...
class Uniform:
    ...
    def __init__(self, numerator, denominator_mode, power, gain):
        if denominator_mode not in ['fan_in', 'fan_out', 'fan_sum', 'fan_avg']:
            raise Exception(
                'Invalid denominator mode, must be one of'
                ' [ fan_in | fan_out | fan_sum | fan_avg ]'
            )
    ...
    def create_variables(self, var_shape, dev_str, fan_out=None, fan_in=None):
        ...
        if self._denominator_mode == 'fan_in':
            if fan_in is None:
                raise Exception(
                    'input_channels must be specified '
                    'for fan_in denominator mode.'
                )
            denom = fan_in
            ...
            if fan_in is None:
                raise Exception(
                    'output_channels must be specified'
                    ' for fan_out denominator mode.'
                )
            denom = fan_out
            ...
            if fan_in is None or fan_out is None:
                raise Exception(
                    'input_channels and output_channels must both be specified '
                    'for fan_sum denominator mode.'
                )
