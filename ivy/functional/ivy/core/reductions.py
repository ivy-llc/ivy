...
def any(x, axis=None, keepdims=False, f=None):
    """
    Tests whether any of the input array elements evaluate to True along a specified axis.

    :param x: input array.
    :param axis: axis or axes along which to perform a logical OR reduction. By default, a logical OR reduction must
        be performed over the entire array. If a tuple of integers, logical OR reductions must be performed over multiple
        axes. A valid axis must be an integer on the interval [-N, N), where N is the rank (number of dimensions) of x.
        If an axis is specified as a negative integer, the function must determine the axis along which to perform a
        reduction by counting backward from the last dimension (where -1 refers to the last dimension). If provided an
        invalid axis, the function must raise an exception. Default: None.
    ...
    :param f: Machine learning framework. Inferred from inputs if None.
    """
    return _cur_framework(x, f=f).any(x, axis, keepdims)
