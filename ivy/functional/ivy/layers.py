...
def depthwise_conv2d_backprop_input(
    x: Union[ivy.Array, ivy.NativeArray],
    filter: Union[ivy.Array, ivy.NativeArray],
    strides: Union[int, Tuple[int], Tuple[int, int]],
    padding: Union[str, List[int]],
    /,
    *,
    ...
    """
    Computes a 2-D depthwise convolution backprop input given 4-D input ``x`` and filters arrays.
    Parameters
    ----------
    x
        Input image *[batch_size,h,w,d]*.
    ...
    >>> x = ivy.random_normal(mean=0, std=1, shape=[1, 28, 28, 3]) #NHWC
    >>> filters = ivy.random_normal(mean=0, std=1, shape=[3, 3, 3]) #HWI (I == d_in)
    >>> y = ivy.depthwise_conv2d_backprop_input(x, filters, (1, 1), 'VALID')
    >>> print(y.shape)
    (1, 26, 26, 3)
    >>> x = ivy.random_normal(mean=0, std=1, shape=[1, 32, 32, 3]) #NHWC
    >>> y = ivy.zeros_like(x)
    >>> filters = ivy.random_normal(mean=0, std=1, shape=[5, 5, 3]) #HWI (I == d_in)
    >>> ivy.depthwise_conv2d_backprop_input(x, filters, [2, 2], 'SAME', out=y)
    >>> print(y.shape)
    (1, 16, 16, 3)
    >>> x = ivy.random_normal(mean=0, std=1, shape=[1, 64, 64, 32]) #NHWC
    >>> filters = ivy.random_normal(mean=0, std=1, shape=[4, 4, 32]) #HWI (I == d_in)
    >>> ivy.depthwise_conv2d_backprop_input(x, filters, [1, 1], 'VALID', out=x)
    >>> print(x.shape)
    (1, 61, 61, 32)
    With :code:`ivy.NativeArray` input:
    >>> x = ivy.native_array( \
    ...
    >>> y = ivy.depthwise_conv2d_backprop_input(x, filters, [1, 1], 'SAME')
    >>> print(y.shape)
    (1, 7, 7, 64)
    With a mix of :code:`ivy.Array` and :code:`ivy.Container` inputs:
    >>> x = ivy.eye(6, 6).reshape((1, 6, 6, 1)) #NHWC
    ...
    >>> b = ivy.array([[1., 1., 1.],\
                       ...
                       [1., 1., 1.]]).expand_dims(axis=-1) / 9.0
    >>> filters = ivy.Container(a = a, b = b)
    >>> y = ivy.depthwise_conv2d_backprop_input(x, filters, 1, 'VALID', dilations=2)
    >>> print(y)
    {
        a: ivy.array([[[[-6.],
                        [0.]],
    ...
    >>> filter = ivy.array([[1., 1., 1.],\
                            [1., -8., 1.], \
                            [1., 1., 1.]]).expand_dims(axis=-1)
    >>> y = ivy.depthwise_conv2d_backprop_input(inp, filter, 1, 'VALID', dilations=2)
    >>> print(y)
    {
        x: ivy.array([[[[-6.],
                        [0.]],
    ...
    """
    return current_backend(x).depthwise_conv2d_backprop_input(
        x, filter, strides, padding, data_format, dilations, out_backprop=out_backprop
    )
