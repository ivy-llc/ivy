...
def tensordot(
    ...
    *,
    ...
    """
    _validate_contraction_modes_for_axes(x1.shape, x2.shape, axes)
    if batched_modes:
        _validate_contraction_modes_for_batches(x1.shape, x2.shape, batched_modes)
    return current_backend(x1, x2).tensordot(
        x1, x2, axes=axes, batched_modes=batched_modes, out=out
    )


def _validate_contraction_modes_for_axes(shape1, shape2, axes):
    if isinstance(axes, int):
        modes1 = list(range(-axes, 0))
        modes2 = list(range(0, axes))
    else:
        modes1, modes2 = _make_tuple_of_2_elem(axes)
    modes1, modes2 = _transform_into_list(modes1, modes2)
    error_message = (
        "Both tensors must have the same number of modes to contract along. "
    )
    _check_equal_length(modes1, modes2, axes, error_message=error_message)

    error_message = (
        "Contraction dimensions must have the same dimensions in both tensors but got"
    )
    _check_same_dimensions(modes1, modes2, shape1, shape2, error_message=error_message)
    _make_negative_dims_positive(modes1, modes2, shape1, shape2)

    return modes1, modes2


def _validate_contraction_modes_for_batches(shape1, shape2, batched_modes):
...
def _transform_into_list(modes1, modes2):
    try:
        modes1 = list(modes1)
    except TypeError:
        modes1 = [modes1]
