...
def percentile(
    ...
    *,
    ...
    interpolation
        ...
        This specifies the interpolation method to use when the desired percentile lies
        between two data points i < j:
        - linear: i + (j - i) * fraction, where fraction is the fractional part of the
        index surrounded by i and j.
        - lower: i.
    ...
    ret
        A (rank(q) + N - len(axis)) dimensional array of same dtype as a, or, if axis
        is None, a rank(q) array. The first rank(q) dimensions index percentiles for
        different values of q.
    ...
    >>> ivy.percentile(a, q, axis=0)
    ivy.array([6.5, 4.5, 2.5])

    >>> ivy.percentile(a, q, axis=1)
    ivy.array([7.,  2.])

    >>> ivy.percentile(a, q, axis=1, keepdims=True)
    ivy.array([[7.],[2.]])

    >>> a = ivy.array([1., 2., 3., 4.])
    >>> q = ivy.array([0.3, 0.7])
    >>> ivy.percentile(a, q, interpolation='lower')
    ivy.array([1., 3.])
    """
