...
def nested_map(
    ...
    if tuple_check_fn(x, tuple) and not isinstance(x, to_ignore):
    ...
    elif (list_check_fn(x, list) or list_check_fn(x, ivy.Array) or isinstance(x, extra_nest_types)) and not isinstance(
        x, to_ignore
    ):
        ...
        if shallow:
            x[:] = ret_list[:]
        return class_instance(ret_list)
    elif (dict_check_fn(x, dict) or dict_check_fn(x, ivy.Container) or isinstance(x, UserDict)) and not isinstance(
        x, to_ignore
    ):
        class_instance = type(x)
        ret = {
