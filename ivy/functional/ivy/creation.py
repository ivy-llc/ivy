...
def arange(
    ...
    (in other words, the interval including start but excluding stop). For integer
    ...
    dtype
        ...
        If  None, the output array data type must be inferred from start, stop and step.
        If  all integers, the output array dtype must be the default integer dtype.
        If one or more have type float, then the output array dtype must be the
        default float dtype
        Default: None.
    device
...
def zeros(
    ...
    *,
    ...
    dtype
       output array data type.
       If ``dtype`` is ``None``,
       the output array data type must be the default floating-point data type.
       Default  ``None``.
    device
       device on which to place the created array. Default: ``None``.
...
def full_like(
    ...
    and having the same ``shape`` as an input array ``x``.
    ...
    -------
    ret:
        an array having the same shape as ``x``
        and where every element is equal to ``fill_value``.
...
def tril(x: Union[ivy.Array, ivy.NativeArray], k: int = 0) -> ivy.Array:
    ...
    k
        ...
        If k = 0, the diagonal is the main diagonal. If k < 0, the diagonal is
        below the main diagonal. If k > 0, the diagonal is above the main diagonal.
        Default: 0.
    ...
    ret
        ...
        x. All elements above the specified diagonal k must be zeroed.
...
def triu(x: Union[ivy.Array, ivy.NativeArray], k: int = 0) -> ivy.Array:
    ...
    ret
        ...
        The returned array must have the same shape and data type as
        x. All elements below the specified diagonal k must be zeroed.
        The returned array should be allocated on the same device as x.
    """
...
def eye(
    ...
    n_cols
        ...
        If None, the default number of columns in the output array is
    ...
    dtype
        output array data type.
        If dtype is None, the output array data type must be the default floating-
...
def linspace(
    ...
    """
    return _cur_framework(start).linspace(start, stop, num, axis, device, dtype,
                                          endpoint)
...
def meshgrid(
    *arrays: Union[ivy.Array, ivy.NativeArray], indexing: Optional[str] = "xy"
    ...
    ret List[array]
        ...
        Accordingly, for the two-dimensional case with input one-dimensional arrays
        of length ``M`` and ``N``, if matrix indexing ``ij``,
        then each returned array must have shape ``(M, N)``,
        and, if Cartesian indexing ``xy``,
        ...
        length ``M``, ``N``, and ``P``, if matrix indexing ``ij``,
        then each returned array must have shape ``(M, N, P)``, and,
        if Cartesian indexing ``xy``,
...
def full(
    ...
    dtype
        output array data type.
        If ``dtype`` is `None`,
        the output array data type must be inferred from ``fill_value``.
        If the fill value is an ``int``,
        the output array data type must be the default integer data type.
        If the fill value is a ``float``,
        the output array data type must be the default floating-point data type.
        If the fill value is a ``bool``, the output array must have boolean data type.
        Default: ``None``.
    device
        device on which to place the created array. Default: ``None``.
...
def from_dlpack(x: Union[ivy.Array, ivy.NativeArray]) -> ivy.Array:
    ...
    from another (array) object with a ``__dlpack__`` method.
    ...
    ret
        ...
        .. admonition:: Note
           ...
           The returned array may be either a copy or a view.
           See :ref:`data-interchange` for details.
    """
