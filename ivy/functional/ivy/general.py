...
def default(x: Any, default_val: Any, catch_exceptions: bool = False, rev: bool = False, with_callable: bool = False) \
    ...
    Parameters
    ----------
    x : value if catch_exceptions=False else callable
...
def match_kwargs(kwargs, *receivers, allow_duplicates=False):
    ...
    receivers : callables and/or classes
...
def stable_divide(numerator: Any, denominator: Any, min_denominator: float = None) -> Any:
    ...
    Parameters
    ----------
    numerator : any valid numerator, including containers
        The numerator of the division.
    denominator : any valid denominator, including containers
...
def stable_pow(base: Any, exponent: Any, min_base: float = None) \
    ...
    Parameters
    ----------
    base : any valid numerator, including containers
        The numerator of the division.
    exponent : any valid denominator, including containers
        The denominator of the division.
...
def queue_timeout():
    """Get the global queue timeout values (in seconds). Default value without this function being called is 10
    seconds. """
    global TIMEOUT
    return TIMEOUT
