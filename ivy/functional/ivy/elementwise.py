...
def real(
    ...
    *,
    ...
    """Tests each element ``x_i`` of the input array ``x`` to take only real part
    from it.
    ...
    -------
    ret
        an array containing test results. An element ``out_i`` is ``real number``
        if ``x_i`` contain real number part only and if it is
        ``real number with complex part also`` then it
