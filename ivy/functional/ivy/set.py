...
def unique_all(x: Union[ivy.Array, ivy.NativeArray]) \
    ...
    """
    Returns the unique elements of an input array ``x``, the first occuring indices for each unique element in ``x``, the indices from the set of unique elements that reconstruct ``x``, and the corresponding counts for each unique element in ``x``.
    ...
    out: Tuple[array, array, array, array]
        ...
        - first element must be the field name ``values`` and must be an array containing the unique elements of ``x``, The array must have the same data type as ``x``.
        - second element must have the field name ``indices`` and must be an array containing the indices (first occurences) of ``x`` that result in ``values``. The array must have the same shape as ``values`` and must have the default array index data type.
        ...
        - fourth element must have the field name ``counts`` and must be an array containing the number of times each unique element occurs in ``x``. The returned array must have the same shape as ``value`` and must have the default array index data type.
        .. note::
