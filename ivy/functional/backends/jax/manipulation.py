...
def clip(x, x_min, x_max, out: Optional[JaxArray] = None):
    if hasattr(x_min,'dtype') and hasattr(x_max,'dtype') and (x.dtype != x_min.dtype or x.dtype != x_max.dtype):
        ...
        elif (jnp.float16 in (x.dtype, x_min.dtype, x_max.dtype) or jnp.float32 in (x.dtype,x_min.dtype,x_max.dtype)) and (jnp.int32 in (x.dtype, x_min.dtype, x_max.dtype) or jnp.uint32 in (x.dtype, x_min.dtype, x_max.dtype) or jnp.uint64 in (x.dtype, x_min.dtype, x_max.dtype) or jnp.int64 in (x.dtype, x_min.dtype, x_max.dtype)):
