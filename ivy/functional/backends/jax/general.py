...
from ivy.functional.backends.jax.device import to_device, _to_array, dev
...
def gather(
    ...
    if device is None:
        device = dev(params)
    return to_device(jnp.take_along_axis(params, indices, axis), device)
...
def one_hot(indices, depth, *, device):
    ...
    return to_device(res.reshape(list(indices.shape) + [depth]), default_device(device))
