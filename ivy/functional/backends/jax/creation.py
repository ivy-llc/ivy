...
def ones_like(
    ...
    if dtype and str:
        ...
        dtype = x.dtype
    return to_device(jnp.ones_like(x, dtype=dtype), device=device)
...
def zeros_like(
    ...
    if not dtype:
        dtype = x.dtype
    return to_device(jnp.zeros_like(x, dtype=dtype), device=device)
...
def empty_like(
    ...
    return to_device(jnp.empty_like(x, dtype=dtype), device=device)
    ...
    *,
    ...
    if isinstance(object_in, ivy.NativeArray) and dtype != "bool":
        ...
        if copy is True:
            ...
            return to_device(jnp.asarray(object_in, dtype=dtype), device=device)
    else:
        dtype = default_dtype(dtype, object_in)

    if copy is True:
        return to_device(jnp.array(object_in, dtype=dtype, copy=True), device=device)
    else:
        return to_device(jnp.asarray(object_in, dtype=dtype), device=device)
...
def eye(
    ...
    *,
    ...
    return to_device(jnp.eye(n_rows, n_cols, k, dtype), device=device)
...
def arange(
    ...
    *,
    ...
    res = to_device(jnp.arange(start, stop, step=step, dtype=dtype), device=device)
    if not dtype:
