...
def overlap_and_add(
    ...
    frame_step = jnp.array(frame_step)
    ...
    if frame_step.dtype not in (jnp.int8, jnp.int16, jnp.int32, jnp.int64):
    ...
    signal_shape = jnp.array(signal.shape)

    # get outer_dimensions [:-2]
    outer_dimensions = jnp.array(signal_shape[:-2])
    ...
    def full_shape(inner_shape):
        if not isinstance(inner_shape, jnp.ndarray):
            inner_shape = jnp.array(inner_shape)
        return jnp.concatenate([outer_dimensions, inner_shape], axis=0)
    ...
    if frame_step and signal.shape is not None and frame_step == signal.shape[-1]:
        ...
        return jnp.reshape(signal, output_shape)
    ...
    # 0 0 0 0 0 0
    paddings = jnp.array(
        [[0, segments], [0, segments * frame_step - frame_length]]
    )  # zero padding for frames
    outer_paddings = jnp.zeros(
        [outer_rank, 2], dtype=jnp.int32
    )  # dummy for outer_rank [0, 0]
    paddings = jnp.concatenate([outer_paddings, paddings], axis=0)
    signal = jnp.pad(signal, paddings, mode="constant", constant_values=0)
    ...
    perm = jnp.concatenate(
        [jnp.arange(outer_rank), outer_rank + jnp.array([1, 0, 2])], axis=0
    )
    signal = jnp.transpose(signal, axes=perm)
