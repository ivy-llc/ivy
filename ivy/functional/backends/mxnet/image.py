...
def gradient_image(x):
    ...
    dy = _ivy.concatenate((dy, mx.nd.zeros(batch_shape + [1, image_dims[1], num_dims], ctx=ctx)), -3)
    # noinspection PyTypeChecker
    dx = _ivy.concatenate((dx, mx.nd.zeros(batch_shape + [image_dims[0], 1, num_dims], ctx=ctx)), -2)
