...
def prod(x: _torch.Tensor,
    ...
    if axis is None:
        ...
        if len(axis) == 0:
            ...
            return _torch.prod(_torch.Tensor(
                [_torch.prod(input=x, dim=i, dtype=dtype, keepdim=keepdims) for i in axis]), dtype=dtype)
