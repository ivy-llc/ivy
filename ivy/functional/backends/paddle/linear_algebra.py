...
def _tensordot_with_batched_modes(x1, x2, axes, batched_modes):
    ...
    x1 = paddle.transpose(x1, batch_modes1 + new_modes1 + modes1)
    x1 = paddle.reshape(x1, (*batch_shape, -1, contraction_dim))
    ...
    x2 = paddle.transpose(x2, batch_modes2 + modes2 + new_modes2)
    x2 = paddle.reshape(x2, (*batch_shape, contraction_dim, -1))

    res = paddle.matmul(x1, x2)
    res = paddle.reshape(res, (*batch_shape, *new_shape1, *new_shape2))

    final_modes = _final_modes(x1, modes1, batch_modes1)
    final_modes += [i for i in range(res.ndim) if i not in final_modes]
    if final_modes:
        res = paddle.transpose(res, final_modes)
    return res
