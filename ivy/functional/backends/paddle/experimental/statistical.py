...
@with_supported_dtypes(
    {"2.5.1 and below": ("float", "int32", "int64", "bool", "complex")},
    backend_version,
)
def median(
    input: paddle.Tensor,
...
@with_supported_dtypes(
    {"2.5.1 and below": ("int64", "float64", "complex")}, backend_version
)
def cummax(
    x: paddle.Tensor,
    /,
...
def __find_cummax(
    ...
    if (
        ...
        and len(x[0].shape) >= 1
        and (
            isinstance(type(x[0]), paddle.Tensor)
            or isinstance(type(x[0]), ivy.data_classes.array.array.Array)
        )
    ):
        if axis >= 1:
    ...
    if isinstance(type(x), paddle.Tensor):
