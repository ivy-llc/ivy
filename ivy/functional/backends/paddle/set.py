...
def unique_inverse(x: paddle.Tensor, /) -> Tuple[paddle.Tensor, paddle.Tensor]:
    ...
    if nan_count > 0:
        ...
        unique_nan = paddle.full(shape=[1, nan_count],
                                 fill_value=float('nan')).cast(x.dtype)
        unique = paddle.concat(input=[unique.astype(
            x.dtype), paddle.reshape(unique_nan, [nan_count])], axis=-1)
...
def unique_values(
    ...
    if nan_count > 0:
        nans = paddle.full(shape=[nan_count],
                           fill_value=float('nan')).cast(unique.dtype)
