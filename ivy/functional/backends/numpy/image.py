...
def gradient_image(x):
    ...
    # BS x H x W x D
    dy = _ivy.concat(
        (dy, _ivy.zeros(batch_shape + [1, image_dims[1], num_dims], device=device, dtype=dtype)), -3
    )
    dx = _ivy.concat(
        (dx, _ivy.zeros(batch_shape + [image_dims[0], 1, num_dims], device=device, dtype=dtype)), -2
    )
