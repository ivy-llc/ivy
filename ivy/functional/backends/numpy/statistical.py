...
def max(
    x: np.ndarray, axis: Union[int, Tuple[int]] = None, keepdims: bool = False, out: Optional[np.ndarray] = None
) -> np.ndarray:
...
def einsum(equation: str, *operands: np.ndarray, out: Optional[np.ndarray] = None) -> np.ndarray:
