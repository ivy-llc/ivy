...
def sort(x: Tensor,
    ...
    if descending:
        return tf.sort(tf.cast(tf.convert_to_tensor(tf.cast(x, dtype=tf.int32))), axis=axis, direction='DESCENDING')
    else:
        return tf.sort(tf.cast(tf.convert_to_tensor(tf.cast(x, dtype=tf.int32))), axis=axis, direction='ASCENDING')
