...
def matrix_rank(
    ...
    *,
    ...
    eps = tf.experimental.numpy.finfo(x.dtype)

    if rtol is not None:
        singular_values = tf.linalg.svd(x, compute_uv=False)
        if atol is None:
            atol = rtol * eps
        return tf.math.count_nonzero(singular_values >= atol)

    return tf.cast(tf.linalg.matrix_rank(x, tol=atol), dtype=tf.int64)
