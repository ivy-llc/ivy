...
def is_variable(x, exclusive=False):
    # CUPY does not support autograd, checking if x is a variable does have any meaning
    # for CUPY. Return False.
    return False
...
def value_and_grad(func):
    logging.warning(
        "CUPY does not support autograd, 'value_and_grad' "
...
def jac(func):
    logging.warning(
        "CUPY does not support autograd, 'jac' "
        "has no effect on the array, as gradients are not supported in the first place."
    )
...
def grad(func):
    logging.warning(
        "CUPY does not support autograd, 'grad' "
        "has no effect on the array, as gradients are not supported in the first place."
    )
