...
def native_sparse_array(
    ...
    *,
    ...
    logging.warning("cupy does not support sparse array natively, None is returned.")
    return None


def native_sparse_array_to_indices_values_and_shape(x):
    logging.warning(
        "cupy does not support sparse array natively, None is returned for \
...
def sinc(x: cp.ndarray, /, *, out: Optional[cp.ndarray] = None) -> cp.ndarray:
    return cp.sinc(x).astype(x.dtype)
