...
def depthwise_conv2d(
    ...
    *,
    ...
    if data_format == "NHWC":
        x = cp.transpose(x, (3, 0, 1, 2))
    else:
        x = cp.transpose(x, (1, 0, 2, 3))
    depth = x.shape[0]
    filters = cp.transpose(filters, (2, 0, 1))
    x = cp.expand_dims(x, -1)
    filters = cp.expand_dims(filters, (-1, -2))
    x_shape = x[0].shape
    ...
    if padding == "VALID":
        out_height = cp.ceil(float(x_shape[1] - filter_h + 1) / float(strides[0]))
        out_width = cp.ceil(float(x_shape[2] - filter_w + 1) / float(strides[1]))
    else:
        out_height = cp.ceil(float(x_shape[1]) / float(strides[0]))
        out_width = cp.ceil(float(x_shape[2]) / float(strides[1]))
    if data_format == "NHWC":
        outputs = cp.empty([x_shape[0], int(out_height), int(out_width), 0], x.dtype)
    else:
        outputs = cp.empty([x_shape[0], 0, int(out_height), int(out_width)], x.dtype)
    for i in range(depth):
        output = conv2d(
            x[i], filters[i], strides, padding, data_format="NHWC", dilations=dilations
        )
        if data_format == "NHWC":
            outputs = cp.append(outputs, output, axis=-1)
        else:
            outputs = cp.append(outputs, cp.transpose(output, (0, 3, 1, 2)), axis=1)
    return outputs
...
def conv3d(
    ...
    *,
    ...
    sub_matrices_w_output_dim = cp.tile(
        cp.expand_dims(sub_matrices, -1), [1] * 8 + [output_dim]
    )
    # B x OD X OH x OW x KD X KH x KW x I x O
    mult = sub_matrices_w_output_dim * filters.reshape(
    ...
    res = cp.sum(mult, (4, 5, 6, 7))
    if data_format == "NCDHW":
        return cp.transpose(res, (0, 4, 1, 2, 3))
    return res
...
def conv3d_transpose(
    ...
    *,
    ...
    if data_format == "NCDHW":
        res = cp.transpose(res, (0, 4, 1, 2, 3))
    return res
...
def conv_general_dilated(
    ...
    *,
    ...
    if data_format == "channel_first":
        x = cp.transpose(x, (0, *range(2, dims + 2), 1))
    ...
    for i, j in zip(
        ...
        # B x OH x OW x O
        res.append(cp.sum(mult, tuple([i for i in range(dims + 1, dims * 2 + 2)])))
    res = cp.concatenate(res, axis=-1)

    if data_format == "channel_first":
        return cp.transpose(res, (0, dims + 1, *range(1, dims + 1)))
    return res
