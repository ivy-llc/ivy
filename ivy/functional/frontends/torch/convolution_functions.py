...
def conv1d(input, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    shape (batch_size, d_in, w) weight: filters of shape (d_out, d_in/groups,
    fw) bias:  optional bias of shape (d_out). Default: None stride: the stride of
    the convolving kernel. Can be a single number or a one-element tuple (sW,
    ). Default: 1 padding: implicit paddings on both sides of the input. Can be a
    string {‘valid’, ‘same’}, single number or a one-element tuple (padW,). Default:
    0 padding='valid' is the same as no padding. padding='same' pads the input so the
    output has the same shape as the input. dilation: the spacing between kernel
    elements. Can be a single number or a one-element tuple (dW,). Default: 1 groups:
    split input into groups, in_channels and out_channels should be divisible by the
