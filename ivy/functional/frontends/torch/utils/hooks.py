                ...
                if self.user_pre_hooks:
                    ...
                    for user_pre_hook in self.user_pre_hooks:
                        ...
                        if actual_len != expected_len:
                            raise RuntimeError(
                                "Backward pre hook returned an invalid number of"
                                f" grad_output, got {actual_len}, but expected"
                                f" {expected_len}"
                            )
                        self.grad_outputs = hook_grad_outputs

                # Special case if no input required gradients,
                # this hook should call the user hook directly
                    ...
                    for user_hook in self.user_hooks:
                        ...
                        if res is not None and not (
                            ...
                            raise RuntimeError(
                                ...
                                " gradient should always return None or None for all"
                                " gradients."
                            )
                    self.grad_outputs = None
