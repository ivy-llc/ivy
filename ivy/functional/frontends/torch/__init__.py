...
def promote_types_of_torch_inputs(
    *args: Union[ivy.Array, Number, Iterable[Number]],
    ...
    if len(set(dtypes)) == 1:
        good_shape = ivy.nested_argwhere(
            args, lambda x: not x.shape == (), stop_after_n_found=1
        )
        good_array = ivy.multi_index_nest(args, good_shape)
        # print(good_array)

        index = ivy.nested_argwhere(args, lambda x: x.shape == ())
        array_vals = ivy.multi_index_nest(args, index)

        def nested_multi(x):
            x = ivy.asarray(
                x,
                dtype=good_array.dtype,
                device=ivy.default_device(item=good_array, as_native=False),
            )
            return x

        ivy.nested_map(array_vals, nested_multi)
    else:
        # type1 = args[0].dtype
        for arg in args:
