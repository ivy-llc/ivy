...
def histc(input, bins=100, min=0, max=0, *, out=None):
    input_ivy = ivy.flatten(
        input
    )  # torch.histc results in a 1D tensor so I flattened it out.
    if min == 0 and max == 0:
        min = ivy.min(input_ivy)
        max = ivy.max(input_ivy)
    if min == max:
...
@to_ivy_arrays_and_back
@with_supported_dtypes({"2.0.1 and below": ("float32", "float64")},
def histc(input, bins=100, min=0, max=0, *, out= None):
    ...
    if min == max:
        min = min - 5.000000e-01
        max = max + 5.000000e-01
