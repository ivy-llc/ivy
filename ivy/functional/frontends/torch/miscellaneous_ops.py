...
def flatten(input, start_dim=0, end_dim=-1):
    ...
    # This for loop is to work out the new shape
    # It is a map Z^n -> Z^(n-1) where
    # (...., a,b, ....)
    # maps to
    # (...., ab, ....)
    # and then do the resize in steps.
    for i in range(start_dim, end_dim):
        new_shape = list(input.shape)
        new_shape[i] = new_shape[i] * new_shape[i + 1]
        # for j in range(len(new_shape)-2, i, -1):
        for j in range(i + 1, len(new_shape) - 1, 1):
