...
def gather(input, dim, index, *, sparse_grad=False, out=None):
    ...
    all_indices = ivy.array(list(ivy.where(ivy.full(index.shape, True), None, None)), dtype='int64')
    gather_locations = ivy.reshape(index, [ivy.prod(ivy.array(index.shape))])

    gather_indices = []
    for axis in range(len(index.shape)):
        if axis == dim:
            gather_indices.append(ivy.array(gather_locations, dtype="int64"))
        else:
            gather_indices.append(ivy.array(all_indices[axis], dtype="int64"))
