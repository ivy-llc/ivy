...
def _from_cv2_frontend_array_to_ivy_array(x):
    if (
...
def _to_ivy_array(x):
    return _from_cv2_frontend_array_to_ivy_array(_native_to_ivy_array(x))


def inputs_to_ivy_arrays(fn: Callable) -> Callable:
    @functools.wraps(fn)
    def _inputs_to_ivy_arrays_cv2(*args, **kwargs):
        """
    ...
    _inputs_to_ivy_arrays_cv2.inputs_to_ivy_arrays = True
    return _inputs_to_ivy_arrays_cv2


def _from_ivy_array_to_cv2_frontend_array(x, nested=False, include_derived=None):
    if nested:
...
def outputs_to_frontend_arrays(fn: Callable) -> Callable:
    @functools.wraps(fn)
    def _outputs_to_frontend_arrays_cv2(*args, **kwargs):
        """
        ...
        return _from_ivy_array_to_cv2_frontend_array(
            ret, nested=True, include_derived={tuple: True}
        )

    _outputs_to_frontend_arrays_cv2.outputs_to_frontend_arrays = True
    return _outputs_to_frontend_arrays_cv2
