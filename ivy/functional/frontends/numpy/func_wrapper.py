    ...
    def new_fn(*args, casting="same_kind", dtype=None, **kwargs):
        ...
        if casting == "no" or casting == "equiv":
            ...
            if ivy.exists(dtype):
                ...
                promoted_type = dtype
            args[0] = ivy.array(args[0], dtype=promoted_type)
            args[1] = ivy.array(args[1], dtype=promoted_type)
        elif casting == "same_kind":
            # TODO: implement!
            if _is_same_kind(ivy.dtype(args[0]), ivy.dtype(args[1])):
                ivy.assertions.check_true(_is_same_kind())
        else:
            pass  # unsafe
