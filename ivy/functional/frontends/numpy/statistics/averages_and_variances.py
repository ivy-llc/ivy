...
def nanpercentile(
    ...
    *,
    ...
    if axis == 1:
        nanlessarrayofarrays = [[t for t in i if not ivy.isnan(t)] for i in a]
        resultarray = [[cpercentile(ii, i) for ii in nanlessarrayofarrays] for i in q]
    else:
        a = ivy.swapaxes(a, 0, 1) if axis == 0 else a
        nanlessarrayofarrays = [[t for t in i if not ivy.isnan(t)] for i in a]
        resultarray = [[cpercentile(ii, i) for ii in nanlessarrayofarrays] for i in q]
    return resultarray
