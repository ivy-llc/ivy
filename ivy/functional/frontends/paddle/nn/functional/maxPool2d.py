...
def check_neg_arr(x,kernel_size,padding,stride,dilation,data_format):
    ...
    if(data_format=="NHWC"):
        ...
        raise ValueError(
            f"data_format value must be either NCHW or NHCW"
        )

    if(isinstance(padding,str)):
        padding = [min(kernel_size[i]//2 , ((input_size[i]-1)*stride[i]-input_size[i]+1+(dilation[i]*(kernel_size[i]-1)))/2) for i in range(2)]

    h_out = int(((input_size[0]+2*padding[0]-(dilation[0]*(kernel_size[0]-1))-1)/stride[0])+1)
    w_out = int(((input_size[1]+2*padding[1]-(dilation[1]*(kernel_size[1]-1))-1)/stride[1])+1)

    if(h_out<=0 or w_out<=0):
        raise RuntimeError(
            f"Given input size: ({n}x{input_size[0]}x{input_size[1]}). Calculated output size: ({n}x{h_out}x{w_out}). Output size is too small"
        )
