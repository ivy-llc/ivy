...
def grid_sample(input, grid, mode="bilinear", padding_mode="zeros"):
    ...
    if mode == "bilinear":
        ...
        indices_tl = grid_floor[..., 1, :, :].clamp(0, H - 1), grid_floor[
            ..., 0, :, :
        ].clamp(0, W - 1)
        indices_tr = grid_floor[..., 1, :, :].clamp(0, H - 1), grid_ceil[
            ..., 0, :, :
        ].clamp(0, W - 1)
        indices_bl = grid_ceil[..., 1, :, :].clamp(0, H - 1), grid_floor[
            ..., 0, :, :
        ].clamp(0, W - 1)
        indices_br = grid_ceil[..., 1, :, :].clamp(0, H - 1), grid_ceil[
            ..., 0, :, :
        ].clamp(0, W - 1)
        ...
        wa = (
            (grid[..., 0, :, :] - indices_tl[1].float())
            * (grid[..., 1, :, :] - indices_tl[0].float())
        ).unsqueeze(1)
        wb = (
            (indices_tr[1].float() - grid[..., 0, :, :])
            * (grid[..., 1, :, :] - indices_tr[0].float())
        ).unsqueeze(1)
        wc = (
            (grid[..., 0, :, :] - indices_bl[1].float())
            * (indices_bl[0].float() - grid[..., 1, :, :])
        ).unsqueeze(1)
        wd = (
            (indices_br[1].float() - grid[..., 0, :, :])
            * (indices_br[0].float() - grid[..., 1, :, :])
        ).unsqueeze(1)

        wa * values_tl + wb * values_tr + wc * values_bl + wd * values_br
    ...
    # Nearest-neighbor sampling
        ...
        if padding_mode == "zeros":
            ...
            sampled_output = torch.where(
                mask_x | mask_y, torch.zeros_like(sampled_output), sampled_output
            )
