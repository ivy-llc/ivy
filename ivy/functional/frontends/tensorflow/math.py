...
_chebyshev_coefficients_for_exp_sqrt_i1 = [
...
def bessel_i1(x, name=None):
    ...
    if ivy.any(mask1) > 0:
        y = (z[mask1] / 2.0) - 2.0
        result[mask1] = (
            _chbevl(y, _chebyshev_coefficients_for_exp_i1, 29)
            * z[mask1]
            * ivy.exp(z[mask1])
        )
    ...
    if ivy.any(mask2) > 0:
        result[mask2] = (
            ivy.exp(z[mask2])
            * _chbevl(
                32.0 / z[mask2] - 2.0, _chebyshev_coefficients_for_exp_sqrt_i1, 25
            )
            / ivy.sqrt(z[mask2])
        )
