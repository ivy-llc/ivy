...
class DType:
    def __init__(self, dtype_int):
        self._ivy_dtype = tf_1_frontend.tensorflow_enum_to_type[dtype_int]

    def __repr__(self):
        return "ivy.frontends.tensorflow_1." + self._ivy_dtype
    ...
    def as_datatype_enum(self):
        return tf_1_frontend.tensorflow_type_to_enum[self._ivy_dtype]
...
def as_dtype(type_value):
    ...
    if ivy.is_native_dtype(type_value):
        return DType(
            tf_1_frontend.tensorflow_type_to_enum[ivy.as_ivy_dtype(type_value)]
        )
    if type_value in tf_1_frontend.tensorflow_enum_to_type:
        return DType(type_value)
    if type_value in tf_1_frontend.tensorflow_type_to_enum:
        return DType(tf_1_frontend.tensorflow_type_to_enum[type_value])
    if type_value is float:
        return DType(1)
    if type_value is bool:
        return DType(10)
    if isinstance(type_value, np_frontend.dtype):
        return DType(tf_1_frontend.tensorflow_type_to_enum[type_value.ivy_dtype])
    if issubclass(type_value, np_frontend.generic):
        return DType(
            tf_1_frontend.tensorflow_type_to_enum[
