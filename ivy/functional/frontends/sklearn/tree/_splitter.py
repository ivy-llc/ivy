...
def node_split_best(
    ...
    if best_split.pos < end:
        ...
        best_split.impurity_left, best_split.impurity_right = criterion.children_impurity(best_split.impurity_left, best_split.impurity_right)
...
class SparsePartitioner:
    ...
    def extract_nnz(self, feature):
        ...
        if (1 - self.is_samples_sorted) * n_samples * ivy.log(
            ...
            self.end_negative, self.start_positive, self.is_samples_sorted = extract_nnz_binary_search(
                X_indices,
                X_data,
                indptr_start,
                indptr_end,
...
class BestSplitter(Splitter):
    ...
    def node_split(
        self, impurity: float, split: SplitRecord, n_constant_features: int
    ):
