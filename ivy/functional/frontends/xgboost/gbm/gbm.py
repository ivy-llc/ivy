...
class GBLinear:
    def __init__(
        ...
        self.scale_pos_weight = 1. if not params["scale_pos_weight"] else params["scale_pos_weight"]
    ...
    def do_boost(self, data, gpair, iter):
        if not self.check_convergence():
            ...
            self.weight = self.updater(gpair, data, self.learning_rate, self.weight,
                                       self.num_feature, iter, self.reg_alpha_denorm, self.reg_lambda_denorm)
...
def _get_gradient(obj, pred, label, scale_pos_weight):
    ...
    w_scaled = ivy.where(label == 1., w * scale_pos_weight, w)
