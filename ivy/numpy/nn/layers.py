...
def conv2d(x, filters, strides, padding, data_format='NHWC', dilations=1):
    ...
    new_strides = (
        x.strides[0:1] + x.strides[1:3] + x.strides[1:3] + x.strides[-1:]
    )
    ...
    sub_matrices_w_output_dim = _np.tile(
        _np.expand_dims(sub_matrices, -1), [1]*6 + [output_dim]
    )

    # B x OH x OW x KH x KW x I x O
    mult = (
        sub_matrices_w_output_dim * filters.reshape(
