...
random_uniform = (
    lambda low=0., high=1., shape=None, dev_str='cpu':
        _np.asarray(_np.random.uniform(low, high, shape))
)
...
def multinomial(
    ...
    samples_stack = [
        _np.random.choice(
            num_classes, num_samples, replace, p=prob[0]
        ) for prob in probs_stack
    ]
    samples_flat = _np.stack(samples_stack)

    return _np.asarray(
        _np.reshape(samples_flat, orig_probs_shape[:-1] + [num_samples])
    )


randint = (
    lambda low, high, shape, dev_str='cpu': _np.random.randint(low, high, shape)
)
