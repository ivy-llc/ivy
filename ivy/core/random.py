...
def random_uniform(low=0.0, high=1.0, shape=None, dev_str='cpu', f=None):
    ...
    In other words, any value within the given interval
...
def multinomial(
    ...
    probs=None, replace=True, dev_str='cpu', f=None
):
    """
    Draws samples from a multinomial distribution.

    Specifcally, returns a tensor where each row contains num_samples indices
    ...
    :param num_samples: Number of independent samples to draw
        from the population.

    :type num_samples: int
    :param batch_size: Number of times to draw a new set of samples
        from the population.
    ...
    :param replace: Whether to replace samples once they've been drawn.
        Default is True.
    ...
    """
    return _get_framework(f=f).multinomial(
        population_size, num_samples, batch_size, probs, replace, dev_str
    )
...
def randint(low, high, shape, dev_str='cpu', f=None):
    ...
    :type low: int
    :param high: One above the highest integer
